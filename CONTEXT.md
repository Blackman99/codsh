...
native buffer instead of appending discarded turns. Official `xai-grok-markdown` renders Markdown, tables, code, mermaid labels, thoughts, and dsh tool cards/diffs. Pretty paint hides inline HTML tags, keeps a ZWJ cluster in one cell, and shows `failed` plus `[error]` for a failed tool. Tab then `l`/`r`/Enter/`y` fold, expand, show raw markdown, open full content, or copy original bytes. `/expand` reprints the last folded block in minimal; `/transcript` opens the exact transcript in `$PAGER`. Display fold state does not rewrite model history or copied bytes. And the active session,
