...
or top-level `force_login_team_uuid` in `requirements.toml`). `/login` reloads the session and connects dsh; `/logout` drops that connection. Empty Enter on first-run reloads
