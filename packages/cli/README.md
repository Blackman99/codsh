...
(to-tickets), and lands them as a Landing wave of parallel worktrees. After every HITL answer the same invocation continues; `/goal` stays disarmed. The two-ring panorama (pinned TTY overlay, teaser, loopback Web) is always on. One spec is bound per run; an adjacent runner-owned snapshot guards the original requirement and sealed track. The [site](https://blackman99.github.io/codsh/)
