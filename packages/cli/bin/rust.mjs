...
export async function launchRust(args) {
  try {
    ...
    const helpOnly = (args.length === 1 && ['--help', '-h', '--version', '-V'].includes(args[0]))
      || (['inspect', 'import', 'login', 'logout', 'setup', 'voice'].includes(args[0]) && args.length > 1 && args.slice(1).every(flag => flag === '--help' || flag === '-h'))
