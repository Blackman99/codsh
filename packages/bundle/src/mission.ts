...
import { parseMainTrack, parseOriginalRequirement } from './plan.ts'
...
export function compileMissionContract(
  ...
  const objective = parseOriginalRequirement(markdown) ?? parseIdea(trackBody) ?? parseRequirementLine(markdown) ?? ''
...
export function classifyPath(path: string): WriteTier {
  ...
  if (normalized.endsWith('mission.contract.json') || normalized.endsWith('.ship.json')) {
    return 'immutable'
  }
  return 'mutable'
}
...
export function classifySpecHeading(heading: string): WriteTier {
  ...
  if (/^(?:original\s+requirement|acceptance\s+criteria)\b/iu.test(name)) return 'immutable'
