...
Read the bound spec, including when this turn verifies its final shipped state; do not search for another unfinished spec. Re-read it before you act. This turn is only the phase that Status names — do not start a later phase; the next /ship turn injects that contract.
...
Use one coordination path for every plan size: the main session coordinates one fresh-context subagent per unblocked ticket. Do not call ralph from /ship; ticket count is not a reason to introduce another coordinator. Ralph remains a separate tool for an explicit user request outside this workflow. Track coordination with todo_write, recording sub-steps of the active ticket rather than duplicating the plan's ticket titles. Give the child the self-contained brief above and the TDD contract in this phase; wait for its result, inspect its actual diff and evidence against the original requirement and Track-N coverage, and use a separate read-only review child for non-trivial work. Re-run the ticket's proof commands against the integrated working tree before marking it complete or committing. A failed, partial, or out-of-scope result leaves the ticket unchecked.
This turn may complete only the runner's Active Ticket. Check its dependencies, delegate its implementation, verify it, commit, and tick only its checkbox. Then stop and return a compact result with evidence paths; do not start another ticket or final delivery. The runner checks the original requirement, sealed sections, approved plan, and progress before injecting the next ticket. Keep Status as landing even after the last ticket; final verification is a separate turn. If cascading re-verification invalidates a checked ticket, untick it and all subsequent tickets, record the failure, and end this turn without implementation so the runner can select the repair ticket.
Enforce a three repair attempts circuit breaker per ticket. The runner stops after two consecutive turns with no checkbox progress and caps total landing turns at three per ticket plus final verification; do not retry indefinitely inside one turn or reset recorded repair failures after resume. A genuine blocker must be recorded in \`## Blocker\` with diagnosis, evidence and the needed decision, then end the turn. Resolve or archive that section only after the blocker is actually addressed. Preserve remaining work and concise coordination notes on disk when stopping or approaching context limits; bare /ship resumes from the ledger, never by adding a nested loop.`

const PHASE_5 = `Phase 5 — done means verified. The workflow ends only when dual-layer DoD passes. Layer 1: Every acceptance criterion passes with you actually running its named command and reading the real output (exit code 0 required). Layer 2: Global repository guardrails (typecheck, tests, and changeset checks) pass with zero new failures against the Phase 3 baseline. After all implementation tickets are checked, run every proof command again yourself — a child's word is a report, not a verification. This turn is final verification only; do not implement tickets. If re-verification fails, untick the failed ticket and all subsequent tickets, record the evidence, keep Status as landing, and stop so the runner can select the repair ticket. First check the integrated diff and requirement-to-Track-to-criterion coverage against the original requirement and sealed spec. A green test suite cannot compensate for a missing requirement or an out-of-scope change. Run an independent read-only review for non-trivial changes; resolve its findings before delivery. Keep full command output in evidence files and read exit codes plus relevant result lines in the parent; do not dump whole test logs into the conversation. Never report a result you did not run, and never weaken a criterion to make it pass; if one cannot be met, say so plainly and why. After Confirm, do not rewrite \`## Main Track\`. A needed contradiction is a blocker, never a silent spec rewrite. Progress (Status, checkboxes, proof logs) remains writable.
