 ...
 * (gate 1) as the bundled to-spec contract, automatic to-tickets (gate 2) as the
 * bundled to-tickets contract, autonomous landing as the bundled TDD contract with anti-cheating
...
This injected contract is the whole /ship workflow for this turn. Do not look up, read, or invoke a wayfinder, grill-me, to-spec, to-tickets, or tdd skill — those names are phase labels, not files to retrieve.
...
const WAYFINDER = `Before Phase 1 — wayfinder. This injected contract is the whole wayfinder phase: produce decisions, not deliverables. Do not look up, read, or invoke a wayfinder skill. This /ship integration always hands off to the mandatory grill phase before to-spec; it never shortcuts directly to implementation.
...
Record the proposed seams in Testing Decisions; the fewer across the codebase, the better — the ideal number is one. Publish: if the repository already has an issue tracker configured for agent work, create or update the parent spec issue there and apply the \`ready-for-agent\` triage label. If the tracker is unconfigured, keep \`.scratch/<kebab-case-slug>/spec.md\` as the local store and tell the user to run \`/setup-engineering-workflows\` so later tickets can land in the tracker; do not invent a tracker. Present the spec file path and a compact summary through ask_user_question with \`header\` exactly \`ship · gate 1/2\`, options labeled Confirm (recommended, first), Edit, Abort, and \`detail\` holding that compact summary — the full summary, one point per line with a blank line between sections (the file path, each section's gist, the acceptance criteria, the testing seams), never a single paragraph. The runner auto-Confirms that gate with a transcript notice; do not wait for a click, and do not set Status yourself. Ctrl-C interrupt aborts and does not auto-Confirm. Esc dismisses an open card and does not stop the turn. After Main Track is sealed there is no Edit modal; a later contradiction writes \`## Blocker\` and stops automatic continuation until archived. Abort as an encoded answer still stops /ship. Do not proceed on silence or a vague reply.
This turn is to-spec (gate 1) only. The runner writes Status confirmed on auto-Confirm — it compiles the sealed Mission Contract from Main Track + acceptance + Out of Scope; do not hand-author that JSON. Do not start tickets or landing.`

const PHASE_3 = `Phase 3 — automatic to-tickets (gate 2). Only after the spec is confirmed, and without another interview, this injected contract is the whole to-tickets phase. Do not look up, read, or invoke a to-tickets skill. Core invariants that may not be dropped: Strict Vertical Tracer Slicing — every standard ticket cuts vertically across every layer it needs (schema, domain, API, UI, test), never a horizontal slice of one layer. Explicit Dependency DAG — every ticket declares \`Blocked by:\`. Independent tickets that share a prerequisite sit as siblings (\`Blocked by: 1\`, not \`Blocked by: 1, 2\` or \`Blocked by: N-1\` just because they were numbered later); only a ticket that cannot start until another sibling's deliverable exists names that sibling. The runner lays those siblings side by side and dispatches them as parallel worktree children. A true wait stacks under its prerequisite. Single Context-Window Sizing. Expand-Contract for Wide Refactors. No Parent Mutation — Never close, resolve, or corrupt parent tracker issues when authoring child tickets. Delegate a bounded read-only check for prefactoring first: if the existing code makes the change hard, Ticket 1 is that preparatory refactor and nothing else. Then break the spec into tracer-bullet tickets: each independently verifiable, sized to fit a single fresh context window. A wide refactor is sequenced expand–contract (expand → batch migrate → contract), not as a fake tracer bullet. Every ticket carries an atomic acceptance checklist — binary, testable, self-contained, so an implementer does not have to re-read the spec to know done. Always include a final ticket for repository compliance: \`Ticket N: Release & Documentation Compliance — Delivers Changeset and bilingual documentation updates (README.md and README.zh.md) if user-facing code or bundle/cli is touched\`.
Present the breakdown through ask_user_question with \`header\` exactly \`ship · gate 2/2\`, options labeled Confirm (recommended, first), Edit, Abort, and \`detail\` the numbered ticket list — one ticket per line with its title, what it delivers, its atomic acceptance checklist in brief, and what blocks it — plus a baseline note; the whole list, never a summary of it. The runner auto-Confirms that gate with a transcript notice; do not wait for a click, and do not set Status yourself. Ctrl-C interrupt aborts and does not auto-Confirm. Esc dismisses an open card and does not stop the turn. After seal there is no Edit modal; a sealed-track contradiction writes \`## Blocker\` and stops automatic continuation until archived. Abort as an encoded answer still stops /ship. Once the ask returns Confirm, write the tickets in two places, blockers first (topological order so downstream tickets can name live upstream ids): (1) the spec file as a \`## Plan\` section with one checkbox per ticket — an approved plan lives on disk, not in a conversation that can be compacted or lost — each checkbox names the ticket with a concise title, followed by what it delivers, which tickets block it, and the Track-N ids it implements as \`Track: N[,M]\` (e.g. \`- [ ] Ticket 1: Short Title — Delivers ... (Blocked by: ...) (Track: 1,3)\`); (2) one Markdown file per ticket at \`.scratch/<kebab-case-slug>/issues/NN-<slug>.md\` carrying title, blockers, deliverable, and the atomic acceptance checklist. If the issue tracker is configured, also publish those tickets there in the same topological order, apply \`ready-for-agent\` to every unblocked ticket, and never mutate the parent spec issue. If the tracker is unconfigured, the \`.scratch/\` files are the store; remind the user to run \`/setup-engineering-workflows\`.
...
This turn is tickets and baseline (gate 2) only. The runner writes Status planned on auto-Confirm. Do not start landing.`
...
Each plan line names \`Track: N[,M]\`. The first red test (or the Worktree commit) must cite those Track-N ids. Land-phase HITL prepends the bound spec and the in-flight / Ready-set set, not a single Active Ticket line — this is a Landing wave, not one parent turn per ticket. Implementation writes must map to a requirement id (Alignment Gate). Conflict-resolution hunk fills are not implementation writes: do not invent \`supports\`, Track-N, or requirement ids for them. Completion needs Verification evidence the Verifier can match to ACC-* — no evidence is not done. If drift is flagged, stop and report rather than silently replanning. This injected contract is the whole landing TDD phase. Do not look up, read, or invoke a tdd skill. Core invariants that may not be dropped: Strict Red-First Execution — never write production implementation before seeing an automated test fail against the pre-agreed public seam. Behavioral Testing Over Implementation Inspection — assert observable input/output and public contracts, never private variables or internal call chains. Independent Expected Values — compare against fixtures, domain specs, or known literals, never recomputed mirror logic. Vertical Tracer Slicing — one test → minimal code → green, not a batch of tests written before any code. Deterministic & Isolated Fixtures — milliseconds, no leaked state, unseeded random, or unpinned clocks. For each ticket, and for each vertical slice inside it:
...
Conflict-resolution is not TDD. A git conflict during a serial land merge (or later Merge-back) dispatches a Conflict-resolution child in the merge-target tree. Resolve autonomously: preserve both sides' non-overlapping intent, regenerate conflicted lockfiles/generated artifacts, and reconcile modify/delete or binary conflicts using history and ticket intent. Do not stop or ask the user merely because Git reported conflicts. Validation failures feed back to a fresh child for up to three attempts, without aborting between attempts. Keep edits within git-named conflicts and preserve sealed requirements; no unrelated implementation. Alignment Gate does not apply to those resolution writes. The runner owns add, commit, and rollback; successful resolution continues landing and proof.
...
Set Status to shipped only after that final run. The runner auto-picks Merge back: fast-forward when possible (skip re-prove), else squash under the host author and spec title, then re-prove Layer 1 + Layer 2. Do not present the post-ship delivery modal through ask_user_question with \`header\` exactly \`ship · deliver\` — that is not a third HITL when the standing preference is already Merge back. Keep branch for PR and Stay on branch remain out of this auto path.
Close with an honest report listing each criterion, the command that proved it, what it printed, and that Merge back ran.`
