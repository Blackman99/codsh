...
Conflict-resolution is not TDD. A git conflict during a serial land merge (or later Merge-back) dispatches one Conflict-resolution child in the merge-target tree only: it fills conflict-marker hunks and keeps both sides' non-overlapping intent. It does not add tests, glue, or chase-green edits. The runner owns add, commit, and abort.
