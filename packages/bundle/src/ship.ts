 ...
 * from idea to shipped, verified code — pre-flight branch isolation, grill-me
 * as the grill-me skill (design tree, frontier rounds), automatic to-spec
 ...
 * proof logging, cascading re-verification, 3-strike circuit breaker, and
 * dual-layer DoD verification.
 *
 ...
 * phase's contract so grill, to-spec, to-tickets, and TDD are not competing
 * for context.
...
const BOOT_CONTINUE = `Continue the /ship workflow. The spec file is the memory, not this conversation. The original requirement, if still typed:

${IDEA}

Read the spec whose Status line is not \`shipped\`. Re-read it before you act. This turn is only the phase that Status names — do not start a later phase; the next /ship turn injects that contract.
...
This turn is pre-flight and grill only. After that handshake, write the spec file (docs/specs/<kebab-case-slug>.md, or the repo's design-document convention) with \`Status: interviewing\`, \`Branch:\` / \`Base-Commit:\` / \`Original-Branch:\` from pre-flight, the one-sentence requirement, and each settled grill decision with its reason — that file is the next turn's memory. Also keep a working copy at \`.scratch/<kebab-case-slug>/spec.md\`. Then stop. Do not synthesize user stories, tickets, or code — the next /ship turn injects to-spec.`

const PHASE_2 = `Phase 2 — automatic to-spec (gate 1). Follow the to-spec skill as the contract, not a summary of it. Core invariants that may not be dropped: Pure Synthesis, Zero Interrogation — Do not interview further; synthesize strictly from what the grill already settled and what the codebase already is. Exhaustive User Stories. Minimal Seams at the Highest Tier. Decisions Over Concrete Snippets. Strict Out-of-Scope Demarcation. First inspect the repository: modules, domain glossary, ADRs, and the highest available integration seam for external verification. Read the spec file first — grill already recorded the settled decisions there. Write or update that spec file inside the repository. Follow the repo's existing convention for design documents if one exists (a specs, rfcs, or ADR directory); otherwise use docs/specs/<kebab-case-slug>.md. Also keep a working copy at \`.scratch/<kebab-case-slug>/spec.md\` so a later to-tickets pass has a local store even when the issue tracker is unconfigured. The spec must stand alone for a reader without this conversation, with these sections in order:
- A metadata header with \`Status:\` line (interviewing, confirmed, planned, landing, shipped) kept current at every phase change, followed by \`Branch: ship/<slug>\`, \`Base-Commit:\`, and \`Original-Branch:\`;
- The one-sentence requirement;
- Problem Statement (from the user's perspective);
- Solution (from the user's perspective);
- Each grill decision with its reason;
...
This turn is tickets and baseline (gate 2) only. On Confirm, set Status to planned then stop. On Abort, stop. Do not start landing — the next /ship turn injects the tdd contract.`
...
Anti-cheating TDD proof logging still applies: red before green, and you actually ran the command. Once every slice on the ticket is green, produce exactly one clean commit per ticket. Never commit broken, failing code to git history. Run the full suite the spec named once at the end of the ticket.
Choose the mechanism by the approved plan's size. If it has at most three tickets and you expect the whole change to fit comfortably in this session's context, implement in-session: track landing with todo_write (recording sub-steps of the active ticket rather than duplicating the plan's ticket titles). If it is larger — four or more substantially independent tickets, or work you expect to exceed what one session can hold — the user running /ship is their explicit request for a fresh-agent Ralph loop: call the ralph tool once, with an objective that names the spec file path as the single source of truth, instructs each round to read the spec from disk (plan, checkboxes, baseline, seams), pick the first unchecked ticket whose blockers are ticked, implement it through the tdd skill at those seams (red witnessed, then minimal green, then the suite), then commit and tick its checkbox, and defines completion as every acceptance criterion in the spec passing. Bound the loop: budget about three rounds per ticket, and instruct it to stop and report rather than continue past two consecutive rounds that tick nothing.
Enforce a three repair attempts circuit breaker per ticket: if a ticket cannot turn green after three attempts, halt autonomous thrashing. In a Ralph loop, stop immediately and emit a structured Blocker report. In the main session, use ask_user_question to surface the failure diagnosis and request human guidance.`

const PHASE_5 = `Phase 5 — done means verified. The workflow ends only when dual-layer DoD passes. Layer 1: Every acceptance criterion passes with you actually running its named command and reading the real output (exit code 0 required). Layer 2: Global repository guardrails (typecheck, tests, and changeset checks) pass with zero new failures against the Phase 3 baseline. After a Ralph loop returns, run every proof command again yourself — the loop's word is a report, not a verification. Never report a result you did not run, and never weaken a criterion to make it pass; if one cannot be met, say so plainly and why. When a decision changes mid-flight, update the spec file first so the file on disk stays the truth.
 ...
 * {@link shipPromptFor} so a later phase does not share context with grill.
 */
export const SHIP_PROMPT = [BOOT, PHASE_0, PHASE_1, PHASE_2, PHASE_3, PHASE_4, PHASE_5].join('\n\n')

/** Which contract a `/ship` turn injects. */
export type ShipPhaseKind = 'grill' | 'spec' | 'tickets' | 'land' | 'done'

/**
 * The prompt for one `/ship` turn: only the phase `status` names.
 *
 * No spec yet is pre-flight and grill. Interviewing is to-spec (gate 1).
 * Confirmed is tickets and baseline (gate 2). Planned or landing is TDD
 * then verification. Each turn stops at its gate so the next injection
 * carries a fresh contract.
 * @param status - the spec's Status line, if a spec exists.
 */
export function shipPromptFor(status: ShipStatus | undefined): string {
  switch (status) {
    case 'interviewing':
      return [BOOT_CONTINUE, PHASE_2].join('\n\n')
    case 'confirmed':
      return [BOOT_CONTINUE, PHASE_3].join('\n\n')
    case 'planned':
    case 'landing':
      return [BOOT_CONTINUE, PHASE_4, PHASE_5].join('\n\n')
    case 'shipped':
      return [BOOT_CONTINUE, PHASE_5].join('\n\n')
    default:
      return [BOOT, PHASE_0, PHASE_1].join('\n\n')
  }
}
