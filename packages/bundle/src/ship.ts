...
Do not call create_goal, update_goal, or get_goal — the runner owns the session compass; the model must not fight it for the slot.
After Confirm, do not rewrite \`## Main Track\`. A needed contradiction is a blocker, never a silent spec rewrite. Progress (Status, checkboxes, proof logs) remains writable.`
...
const PHASE_5 = `Phase 5 — done means verified. The workflow ends only when dual-layer DoD passes. Layer 1: Every acceptance criterion passes with you actually running its named command and reading the real output (exit code 0 required). Layer 2: Global repository guardrails (typecheck, tests, and changeset checks) pass with zero new failures against the Phase 3 baseline. After a Ralph loop returns, run every proof command again yourself — the loop's word is a report, not a verification. Never report a result you did not run, and never weaken a criterion to make it pass; if one cannot be met, say so plainly and why. After Confirm, do not rewrite \`## Main Track\`. A needed contradiction is a blocker, never a silent spec rewrite. Progress (Status, checkboxes, proof logs) remains writable.
...
export function shipPromptFor(status: ShipStatus | undefined, opts: ShipPromptOpts = {}): string {
  ...
  switch (kind) {
    ...
    case 'tickets':
      ...
      break
    case 'land':
      prompt = [BOOT_CONTINUE, PHASE_4, PHASE_5].join('\n\n')
      break
    case 'done':
      prompt = [BOOT_CONTINUE, PHASE_5].join('\n\n')
      break
    default:
      prompt = [BOOT, PHASE_0, PHASE_1].join('\n\n')
  }
  if (prompt.includes('$GOAL_ID')) prompt = prompt.replaceAll('$GOAL_ID', opts.goalId ?? '')
  if (opts.track && kind !== 'grill') prompt = `${opts.track}\n\n${prompt}`
  return prompt
}
