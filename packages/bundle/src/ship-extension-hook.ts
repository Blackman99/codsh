 ...
 * prints the `/ship` command markdown the build writes beside it.
 * @module codsh-bundle/src/ship-extension-hook
 */

import { readFileSync } from 'node:fs'
import { handleShipHook, shipExtensionCommand } from './ship-extension.ts'
...
function main(): number {
  ...
  const out = handleShipHook({ event: process.env.GROK_HOOK_EVENT ?? '', payload, dataDir, cwd })
