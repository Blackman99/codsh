...
describe('Mission Contract control plane e2e', () => {
  it('seals the contract, halts on a rewritten Main Track, and lands only the Active Ticket', async () => {
    ...
    const land = mkdtempSync(join(tmpdir(), 'mission-e2e-'))
    const landPath = writeSpec(land, SEALED.replace('Status: interviewing', 'Status: planned'))
  ...
  it('Verifier blocks completion without evidence and clears premature plan ticks', async () => {
    ...
    expect(onDisk).toMatch(/^Status:\s*landing\b/imu)
    expect(onDisk).toMatch(/- \[ \] Ticket 1/)
    expect(onDisk).toMatch(/- \[ \] Ticket 2/)
    expect(onDisk).not.toMatch(/- \[x\] Ticket 1/i)
    expect(onDisk).not.toMatch(/- \[x\] Ticket 2/i)
