...
- 10b27cc: `/ship` continues after HITL, dispatches a Landing wave, and always shows the two-ring panorama (`/goal` stays disarmed). The Web panorama is one loopback URL for the TTY session: the page polls the live graph and is not rebound when `/ship` continues or a spec appears. Decision tickets join the inner ring as soon as they are written (a published map pointer is not a ticket and does not hide local wayfinder files); landing tickets join the outer ring from the approved plan. Phase contracts are the injected prompt; `/ship` does not look up wayfinder, grill-me, to-spec, to-tickets, or tdd skills.
...
- 698f622: Esc from a Child view restores the covered parent transcript — history, scroll offset, and open folds — instead of replaying the log from scratch and landing at the tail.
- 698f622: Select chrome-row text with a drag and copy it on release, the same gesture the transcript, the input box, and the status row already use.
- 698f622: Stop a running turn with Ctrl-C. Escape still dismisses overlays, readers, and Child views, and no longer cancels the agent.
- 698f622: Paint a terminal tool card that exited non-zero or was killed as a failed row (`✗`), not a green pass.
...
- 698f622: Clear completed ship status rows after verified delivery, including the plan, ticket counts, old todo readout, and settled subagent readout. Keep history and the final Web graph available, preserve running or blocked work, and prevent later events from restoring completed progress or applying the retired mission guard.
- 698f622: A live Conflict-resolution child can fill merge hunks without Alignment Gate refusing the writes as unmapped landing work. Invented `supports`, missing Track mappings, and path aliases no longer stall `/ship` on leftover conflict markers.
- 698f622: Keep `/ship` in the same invocation after Gate 2 while a Landing wave is in flight: the working line stays on, the panorama teaser reports `in-flight`, and each landing child receives the TDD contract. Do not invent a worktree directory or dispatch when `git worktree add` fails.
- 698f622: Lay out landing tickets that share a prerequisite side by side, stack only a true wait under that ticket, and dispatch those siblings as parallel worktree children. A numbered `Blocked by: 1, 2` next to `Blocked by: 1` is a fan-out, not a second wait.
- 698f622: Align parallel landing subagent writes with their respective ticket in the Alignment Gate, and register landing child sessions in the subagent roster with start and end lifecycle events so they appear in the subagent roster list (`Ctrl+H`) and status chrome.
...
- 698f622: Fix false `/ship` snapshot-change stops after Gate 1 by comparing saved field values independently of JSON key order, while continuing to reject missing, corrupt, or changed snapshots.
  
  Recognize local Wayfinder `decision-NN-slug.md` tickets and Markdown-list metadata so each ticket and its status appear in the live TTY and web panorama. Share the filename and Status handling with claim recovery, preserve closed decisions, and specify the local ticket format in the injected Wayfinder contract. Stack the web graph and details on narrow screens so the graph remains usable.
- 698f622: Keep `/ship` plan and land-chip chrome following spec checkboxes after a mid-run interrupt or idle stop, so a later typed continue still updates `plan k/n` without another `/ship`.
- 698f622: Remove the duplicate Ticket claims counts from the Web panorama sidebar. Claim colors stay in the Node status legend.
- 698f622: Draw ticket relations as orthogonal lanes with staggered exits and entries. Skipped-rank edges split left and right of the phase so concurrent prerequisites no longer share one horizontal or vertical run.
- 698f622: Cluster each decision ticket with its answers: the ticket sits on the left, answers stack on the right, and prerequisite arrows stay in the ticket lane instead of looping around the phase.
- 698f622: Make the Ship Web flowchart more compact while reserving space for arrows and labels. Route cross-phase and long dependencies outside node columns, and animate the current phase, its claimed tickets, and supported incoming flow. Stop motion for completed or unrecorded phases, connection loss, and reduced-motion preferences without inventing individual step execution.
- 698f622: The Web panorama page title and heading use the typed `/ship` idea instead of a fixed Ship Flow label.
- 698f622: Give Wayfinder continuation answers a join from each decision column instead of a line from the last workflow step, size CJK answer cards by display width, and space Landing DAGs so skipped-rank prerequisite edges leave the columns instead of crossing tickets.
- 698f622: Lay out sibling dependents of one prerequisite in the same rank, each with its own ticket/answer cluster, so the flowchart forks on a short path. Do not stack those tickets into one column, wrap the prerequisite around the phase, invent research-ticket user answers, or repeat an unlinked copy of a question already shown on its ticket.
- 698f622: Smooth viewport transitions when navigating workflow steps and context items in Ship Web panorama.
- 698f622: Draw the parent-ticket-to-answer connection as a straight horizontal line at ticket mid-height, even when the answer card is taller, instead of an elbow between the two card centers.
- 698f622: `/ship` no longer opens the Web panorama in a browser. The loopback URL is pinned on the panorama teaser and overlay title; a pipe still prints it once.
...
- 698f622: Select status-row text with a drag and copy it on release, the same gesture the transcript and the input box already use.
- 698f622: Draw one connecting `│` down every transcript row — prompts, thinking, tools, answers, notices, and blank separators — and colour it by what the row is. Drop the mixed `›` / `✻` / `·` gutter glyphs so the left rail no longer breaks between blocks.
- 698f622: Fold thinking by default and drop the tool-card highlight. A thought still streams as a `thinking…` head that becomes its clock, but the deliberation stays behind a click or Ctrl+O; tool rows keep their one-line form without a panel fill or amber title.
- 698f622: Animate an in-progress thought. The `thinking…` head ticks the same Braille frames as the working line, and the hint row says `thinking` until the clock lands.
- 698f622: Quiet the tool-row column. A finished success bullet is dim instead of green, so the trailing tick carries the pass; consecutive one-line cards on a TTY open with a blank between them so a long run is readable.
- 698f622: Use the same filled background and text inset for canned command prompts (`/ship`, `/init`, and custom commands) as ordinary user messages. Keep streaming and collapsed thinking to one row, retaining padded reasoning only when expanded.
...
- 923fd48: A running in-process subagent is a view: click the card as soon as the child exists, watch its transcript stream, Esc pops one level

### Patch Changes

- 72bf5c1: fix(ship): stop stacking the plan and todo readouts while a Ralph round runs
...
- 3f0ff88: feat(queue): merge typed-ahead prompts into one message, run queued `!` lines in order, manage the queue on Ctrl+Q, and steer the running turn
...
- b13011d: fix(ship): the gate modal wraps the spec summary and ticket list instead of cutting each line at the frame
...
- 1050a18: feat(markdown): render the inline HTML an answer carries — `<font color>`, `<span style>`, emphasis tags, `<br>`, entities
...
- 5438258: fix(surface): measure wrapped and truncated text by grapheme cluster, so a `🎙️` cell no longer shears a table
