// Modified for codsh: offline palette and narrow-window shortcut clipping.
...
pub fn render_menu(
    ...
    for (y, (i, (key, label))) in (menu_centered.y..).zip(items.iter().enumerate()) {
        ...
        let key = crate::theme::truncate_str(key, menu_centered.width as usize);
        let key_width = cols(&key);
        // The key sits at the right edge, so the label is cut to leave room for it.
        let label = crate::theme::truncate_str(
            label,
