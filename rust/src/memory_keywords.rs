//! Recall keywords for the first-turn memory search (issue #186 follow-up).
//!
//! STOP_WORDS and `extract_keywords` (with their tests) are copied verbatim
//! from crates/codegen/xai-grok-memory/src/query_expansion.rs of grok-build at
//! the commit pinned in `rust/upstream/import.json` (Apache-2.0, see
//! `rust/upstream/LICENSE`). The reference joins these keywords with ` OR `
//! and ranks the FTS5 hits by BM25; `memory::injection_block_for` does the
//! same.
//!
//! The pipeline:
//! ```text
