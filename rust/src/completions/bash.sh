...
_codsh_rust_complete() {
  ...
  local commands="help completions inspect import feedback plugin login logout setup voice sessions dashboard"
  local flags="--help -h --version -V --continue --resume --fork-session --session-id -s --model -m --effort --reasoning-effort --cwd -p --single --prompt-file --prompt-json --verbatim --tools --disallowed-tools --max-turns --rules --append-system-prompt --system-prompt-override --system-prompt --allow --deny --allowedTools --disallowedTools --always-approve --yolo --permission-mode --output-format --trust --revoke-trust --minimal --fullscreen"
  case "$prev" in
    completions)
