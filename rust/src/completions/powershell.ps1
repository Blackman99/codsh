...
Register-ArgumentCompleter -CommandName codsh -ScriptBlock {
  ...
  $commands = @('help', 'completions', 'inspect', 'import', 'feedback', 'plugin', 'login', 'logout', 'setup', 'voice', 'web', 'sessions', 'dashboard', 'export', 'share', 'du', 'disk-usage', 'usage', 'memory', 'worktree', 'doctor', 'wrap')
  $flags = @(
