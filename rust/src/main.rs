...
enum LaunchMode {
    Help,
    Version,
    /// `usage <session-id> [turn]` (ticket 65): the persisted usage ledger.
    Usage {
        session_id: String,
        turn: Option<u64>,
    },
    ...
    ImportSessions(session_migrate::SessionImportFlags),
    ...
    /// `codsh --rust doctor [--json]` / `doctor fix [<id>...] [--yes]` (ticket 155).
    Doctor(doctor::Cli),
    Memory(Vec<String>),
...
struct Launch {
    ...
    no_memory: bool,
    /// `--memory-flush`: after the -p turn, flush it to the session log and
    /// fail unless something was written (ticket 186).
    memory_flush: bool,
...
fn deferred_plain_flag(arg: &str) -> Option<String> {
    ...
    match name {
        ...
        "--experimental-memory" => named(
            "the reference v2 memory store is not implemented in this client; the legacy store is used",
        ),
...
fn parse_launch_inner(args: &[String]) -> io::Result<Launch> {
    ...
    let mode = match rest_flags.as_slice() {
        ...
        ["import", "sessions", flags @ ..] => {
            LaunchMode::ImportSessions(session_migrate::parse_flags(flags)?)
        }
        ...
        ["image"] => LaunchMode::Image {
            command: ImageCommand::Help,
            json: false,
        },
        ...
        ["usage"] => {
            return Err(io::Error::other(
                "missing session id; use codsh --rust usage <SESSION_ID> [TURN]",
            ));
        }
        ["usage", "--help" | "-h", ..] => LaunchMode::UsageHelp,
        ["usage", session_id] => LaunchMode::Usage {
            session_id: (*session_id).to_string(),
            turn: None,
        },
        ["usage", session_id, turn] => LaunchMode::Usage {
            session_id: (*session_id).to_string(),
            turn: Some(turn.parse::<u64>().map_err(|_| {
                io::Error::other(format!(
                    "invalid turn '{turn}'; use codsh --rust usage <SESSION_ID> [TURN]"
                ))
            })?),
        },
        ["usage", ..] => {
            return Err(io::Error::other(
                "too many arguments; use codsh --rust usage <SESSION_ID> [TURN]",
            ));
        }
        ...
        ["doctor", flags @ ..] => {
            LaunchMode::Doctor(doctor::parse_cli(flags).map_err(usage_error)?)
        }
...
/// Ticket 187: `image generate|edit|list|run`. The dsh plugin uses `run`,
/// which reads one JSON job on stdin (a data URL can exceed argv limits).
#[derive(Clone, Debug, PartialEq, Eq)]
enum ImageCommand {
    Help,
    Job(image_gen::Job),
...
fn parse_image(flags: &[&str]) -> io::Result<LaunchMode> {
    let mut json = false;
    let mut help = false;
    let mut action: Option<&str> = None;
    let mut prompt: Option<String> = None;
    let mut aspect_ratio = "auto".to_string();
    ...
    while index < flags.len() {
        match flags[index] {
            ...
            "--prompt" => prompt = Some(value(&mut index, "--prompt")?),
            "--aspect-ratio" => aspect_ratio = value(&mut index, "--aspect-ratio")?,
            "--image" => images.push(value(&mut index, "--image")?),
            "--session-dir" => {
                session_dir = Some(PathBuf::from(value(&mut index, "--session-dir")?))
            }
            word @ ("generate" | "edit" | "list" | "run") if action.is_none() => {
                action = Some(word);
            }
            other => {
                return Err(usage_error(format!(
                    "unsupported image option {other}; use codsh --rust image --help"
                )));
            }
        }
        index += 1;
    ...
    let need_dir = || {
        session_dir.clone().ok_or_else(|| {
            usage_error("missing --session-dir; image results are saved in a session folder")
        })
    };
    let command = match action.unwrap_or_default() {
        "run" => ImageCommand::Run,
        "list" => ImageCommand::List {
            session_dir: need_dir()?,
        },
        kind => ImageCommand::Job(image_gen::Job {
            kind: if kind == "edit" {
                image_gen::JobKind::Edit
            } else {
                image_gen::JobKind::Generate
            },
            prompt: prompt.ok_or_else(|| usage_error("missing --prompt"))?,
            aspect_ratio,
            images,
            session_dir: need_dir()?,
        }),
    };
    Ok(LaunchMode::Image { command, json })
}

fn image_help() -> &'static str {
    "Generate or edit an image through the configured substitute\n\nUsage: codsh --rust image generate --prompt <text> --session-dir <dir> [--aspect-ratio <r>] [--json]\n       codsh --rust image edit --prompt <text> --image <path|file://|data:> [--image ...] --session-dir <dir> [--aspect-ratio <r>] [--json]\n       codsh --rust image list --session-dir <dir> [--json]\n\n[models] image_gen names a [model.<id>] with base_url, supports_image_generation = true, and protocol = \"xai\" (default; the reference JSON body) or \"openai\" (OpenAI Images: size, multipart image[] edits). [models] image_edit, or supports_image_edit = true on the same service, enables edits. env_key / api_key are optional: a keyless local service is allowed, a named but empty env_key is an error. image_size (openai long edge, default 1024) and timeout_secs (default 300) are per service.\nfeatures.image_gen / GROK_IMAGE_GEN turn generation off; features.image_edit is a requirements-only pin and GROK_IMAGE_EDIT the switch; GROK_IMAGE_GEN_MODEL_OVERRIDE / GROK_IMAGE_EDIT_MODEL_OVERRIDE change the request model; tools.media_gen.max_parallel_image_gen_calls (default 4) caps calls per model step.\nOfficial hosts are refused. A reply that is not image bytes, a URL instead of b64_json, a refusal, a timeout, or a cancel (SIGINT/SIGTERM) saves nothing. A result is saved atomically as <session-dir>/images/<n>.<ext> (png, jpg, webp, gif) and never replaces an existing file. codsh does not know the service's price; every call is one request to the configured host."
}
...
fn run_image(
    ...
    let job = match command {
        ...
        ImageCommand::List { session_dir } => {
            if json {
                let images: Vec<serde_json::Value> = image_gen::list_images(session_dir)
                    .iter()
                    .map(|image| {
                        serde_json::json!({
                    ...
                    .collect();
                println!("{}", serde_json::json!({ "ok": true, "images": images }));
            } else {
                println!("{}", image_gen::listing(session_dir));
            }
            return Ok(());
        }
        ImageCommand::Job(job) => job.clone(),
        ImageCommand::Run => {
            let mut input = String::new();
            io::Read::read_to_string(&mut io::stdin(), &mut input)?;
            match image_job_from_json(&input) {
                Ok(job) => job,
                Err(message) => {
                    return image_failure(json, &image_gen::ImageError::Invalid(message), loaded);
                }
            }
        }
    };
    let cancelled = image_gen::cancel_flag();
    ...
    match image_gen::run_job(
        ...
        Ok(outcome) => {
            if json {
                println!("{}", outcome.json(job.kind));
            } else {
                println!("{}", outcome.text(job.kind));
            }
            Ok(())
        }
        Err(error) => image_failure(json, &error, loaded),
    }
}

fn image_job_from_json(input: &str) -> Result<image_gen::Job, String> {
    let value: serde_json::Value =
        serde_json::from_str(input).map_err(|error| format!("invalid image job JSON: {error}."))?;
    let text = |key: &str| value.get(key).and_then(serde_json::Value::as_str);
    let kind = match text("tool") {
        Some(image_gen::IMAGE_GEN_TOOL) => image_gen::JobKind::Generate,
        Some(image_gen::IMAGE_EDIT_TOOL) => image_gen::JobKind::Edit,
        other => return Err(format!("unknown image tool {other:?}.")),
    };
    let images = match value.get("image") {
        ...
        Some(serde_json::Value::Array(items)) => items
            ...
            .map(|item| {
                item.as_str()
                    .map(str::to_string)
                    .ok_or_else(|| "each image reference must be a string.".to_string())
            })
            .collect::<Result<_, _>>()?,
        Some(_) => return Err("image must be a list of references.".into()),
    };
    Ok(image_gen::Job {
        kind,
        prompt: text("prompt").unwrap_or_default().to_string(),
        aspect_ratio: text("aspect_ratio").unwrap_or("auto").to_string(),
        images,
        session_dir: PathBuf::from(text("sessionDir").unwrap_or_default()),
    })
}

fn image_failure(
    json: bool,
    error: &image_gen::ImageError,
    loaded: &config::EffectiveConfig,
) -> io::Result<()> {
    let message = error.to_string();
    let cancelled = matches!(error, image_gen::ImageError::Cancelled);
    if json {
        println!(
            "{}",
            serde_json::json!({
                ...
                "cancelled": cancelled,
                "text": "",
                "disclosure": image_gen::disclosure(&loaded.images),
            })
        );
    } else {
        eprintln!("{message}");
...
/// A plugin enable, disable, update, or removal changes the hooks, agent
/// types, and plugin MCP servers dsh read at start. Rescan assets, and when those differ from what
...
fn replace_if_plugin_runtime_changed(
    ...
    // Plugin MCP servers (ticket 204): dsh mounts servers with the session,
    // so a plugin that was disabled, updated, or removed keeps its tools
    // until the child is replaced; one that was enabled is not mounted yet.
    let mcp_changed = client.as_ref().is_some_and(|active| {
        mcp::plugin_mounts_stale(active.mounted_mcp_plan(), active.mcp_plan_path(), effective)
    });
    if mcp_changed || config::plugin_runtime_changed(effective, extra_env) {
...
fn runtime_apply(effective: &config::EffectiveConfig) -> RuntimeApply {
    ...
    // Monitors (ticket 176): the interactive client only, like the scheduler.
    extra_env.extend(background::monitor_env());
...
fn connect(
    ...
    if !matches!(mode, LaunchMode::Plain { .. }) {
        // The TUI answers MCP elicitations on its card.
        client.set_elicitation_surface("tui");
    }
    client
...
fn apply_session_catalog_slash(
    ...
    match action {
        ...
        CatalogSlash::Info => {
            ...
            if let Some(line) = session_migrate::provenance_line(&effective.dsh_home, &session_id) {
                hint.push('\n');
                hint.push_str(&line);
            }
            // The usage line comes from the same ledger as /usage (ticket 65).
            if let Some(active) = client.as_ref() {
                let base = hint.clone();
                match usage_state.request(active, UsagePurpose::Info(base.clone())) {
                    Ok(()) => *hint = format!("{base}\nUsage: reading…"),
                    Err(error) => *hint = format!("{base}\nUsage: unavailable ({error})"),
                }
            }
            last_error.clear();
            composer.set_text("");
...
fn handle_catalog_overlay_key(
    ...
    memory_session_on: &mut Option<bool>,
    memory_injected: &mut bool,
    memory_running: Option<(&'static str, u64)>,
) -> io::Result<bool> {
    match overlay {
        Overlay::SessionPick {
            hits,
        ...
        Overlay::Memory(browser) => {
            ...
            match memory_ui::handle_key(browser, &store, mapped) {
                ...
                Some(memory_ui::Action::Copy(path)) => {
                    let shown = if browser.pane == memory_ui::Pane::Diagnostics {
                        "memory diagnostics".to_string()
                    } else {
                        path.clone()
                    };
                    let report = clipboard::copy_live(&path, &effective.grok_home);
                    *hint = if report.delivered() {
                        format!("{shown}: {}", report.message)
                    } else {
                        format!("{} (the path stays on screen)", report.message)
                    };
                }
                Some(memory_ui::Action::Diagnostics) => {
                    // Reads files and counters only; nothing is captured.
                    let state = if effective.memory.force_off() {
                        "off (force-disable)"
                    } else if browser.session_enabled {
                        "on"
                    } else if memory_session_on.is_some() {
                        "off (this session)"
                    } else {
                        "off (config)"
                    };
                    ...
                    browser.show_diagnostics(memory_capture::diagnostics(
                        &store,
                        &effective.memory_capture,
                        state,
                        memory_running,
                        session.as_deref(),
                    ));
...
struct StatusView<'a> {
    ...
    /// A tool-specific approval line (ticket 187 image calls); None keeps the generic one.
...
fn status_line(view: StatusView<'_>) -> String {
    ...
    let mut body = match client {
        ...
        Some(client) if awaiting_approval => format!(
            "Connected to dsh ACP session {}{tag}.\n{}",
            client.session_id.as_deref().unwrap_or("unknown"),
            approval.unwrap_or_else(|| "Allow this dsh file tool? y=allow once  n=reject".into())
        ),
...
fn paint_interaction(
    ...
    } else if let Some(card) = ask.elicit.as_ref() {
        elicit_ui::render(frame, above, card, theme);
    }
}
...
fn copy_selected_original(turns: &[Turn], nav: &NavState, home: &Path, hint: &mut String) {
    ...
    *hint = clipboard::copy_live(&original, home).message;
}
    ...
    turns
        ...
        .map(|turn| content::TurnView {
            ...
            permission: turn.permission.as_ref().map(|permission| {
                ...
                let name = tool.map(|tool| tool.title.as_str()).unwrap_or("tool");
                if let Some(tool) = tool.filter(|tool| image_gen::is_image_title(&tool.title)) {
                    // Ticket 187: an image call names what is sent where.
                    return format!(
...
fn run_import_sessions(flags: &session_migrate::SessionImportFlags) -> io::Result<()> {
    ...
    let env: std::collections::BTreeMap<String, String> = std::env::vars().collect();
    let legacy = session_migrate::legacy_home(&env).map_err(io::Error::other)?;
    let code = session_migrate::run(flags, &dsh_home, &legacy)?;
    if code != 0 {
        std::process::exit(code);
    }
    Ok(())
...
struct Intervene {
    ...
    /// Background memory requests: `/flush`, `/dream`, the idle flush, and
    /// the automatic Dream (ticket 186).
    memory: memory_capture::Runner,
    /// Memory store of `(grok_home, cwd)`, opened once.
    memory_store: Option<(PathBuf, PathBuf, memory::Store)>,
    /// The memory-log probe and the memory_v2 notice were shown.
    memory_notices_shown: bool,
    /// Session usage (ticket 65): ledger requests waiting for dsh, the
    /// status-line figures of the current session, and when to refresh them.
    usage: UsageState,
}

/// What a usage ledger request is for.
#[derive(Debug, Clone, PartialEq, Eq)]
enum UsagePurpose {
    /// `/usage` or `/cost`: the reference block.
    Show,
    /// `/session-info`: its text plus the usage line.
    Info(String),
    /// The status line's session figures.
    Status,
}
...
struct UsageState {
    next: u64,
    pending: Vec<(String, UsagePurpose)>,
    /// The session the status figures belong to.
    session: Option<String>,
    status: Option<usage::StatusUsage>,
    /// A refresh is due (session change, a turn settled).
    refresh: bool,
    /// A turn ran since the last refresh; refresh once it settles.
    after_turn: bool,
}

impl UsageState {
    fn request(&mut self, client: &AcpClient, purpose: UsagePurpose) -> Result<(), String> {
        let session = client
            .session_id
            .clone()
            .ok_or_else(|| "the session has not started".to_string())?;
        if client.remote {
            return Err(
                "usage is read from the local dsh session log; this session is remote".into(),
            );
        }
        if !client.control_ready() {
            return Err(client.control_unavailable());
        }
        self.next += 1;
        let id = format!("usage-{}", self.next);
        client.send_control(&usage::request_message(&id, &session, None))?;
        self.pending.push((id, purpose));
        Ok(())
    }

    /// Track the session and turn state; ask for fresh status figures when a
    /// session starts or a turn settles (failed and interrupted turns too).
    fn sync(&mut self, client: Option<&AcpClient>, inflight: bool) {
        let session = client.and_then(|active| active.session_id.clone());
        if session != self.session {
            self.session = session;
            self.status = None;
            self.refresh = self.session.is_some();
        }
        if inflight {
            self.after_turn = true;
        } else if self.after_turn {
        ...
        if !self.refresh
            || self
                .pending
                .iter()
                .any(|(_, purpose)| *purpose == UsagePurpose::Status)
        {
            return;
        }
        if let Some(active) = client
            ...
            && !active.remote
            && self.request(active, UsagePurpose::Status).is_ok()
        {
            self.refresh = false;
        }
    }

    /// Apply one ledger answer. Returns the hint text for `/usage` and
    /// `/session-info`.
    fn accept(&mut self, id: &str, outcome: Result<usage::Ledger, String>) -> Option<String> {
        let at = self.pending.iter().position(|(pending, _)| pending == id)?;
        let (_, purpose) = self.pending.remove(at);
        if let Ok(ledger) = &outcome
            && self.session.as_deref() == Some(ledger.session_id.as_str())
        {
            self.status = Some(usage::StatusUsage::from_summary(&ledger.session));
        }
        match (purpose, outcome) {
            (UsagePurpose::Show, Ok(ledger)) => Some(usage::block_text(&ledger.session)),
            (UsagePurpose::Show, Err(error)) => Some(usage::unavailable_text(&error)),
            (UsagePurpose::Info(base), Ok(ledger)) => Some(format!(
                "{base}\n{}",
                usage::session_info_line(&ledger.session)
            )),
            (UsagePurpose::Info(base), Err(error)) => {
                Some(format!("{base}\nUsage: unavailable ({error})"))
            }
            (UsagePurpose::Status, _) => None,
        }
    }
}
...
struct Interact {
    ...
    /// Ticket 168: the MCP elicitation card, the ones waiting behind it, and
    /// the bridge the MCP proxies ask through.
    elicit: Option<elicit_ui::ElicitCard>,
    elicit_waiting: std::collections::VecDeque<mcp_bridge::Elicitation>,
    bridge: mcp_bridge::SurfaceWatch,
    /// `/mcps auth <name>` running in the background.
    mcp_login: Option<McpLogin>,
    /// A sign-in or sign-out finished: restart the servers when idle.
    mcp_restart: Option<(String, String)>,
}
...
struct McpLogin {
    ...
    updates: std::sync::mpsc::Receiver<McpLoginUpdate>,
}
...
/// Start `/mcps auth <name>` in the background (a second one replaces the first).
fn start_mcp_login(ask: &mut Interact, discovery: mcp::Discovery, name: &str) {
    ...
    std::thread::spawn(move || {
        let env: std::collections::BTreeMap<String, String> = std::env::vars().collect();
        let url_tx = tx.clone();
        let result = mcp::login(
            &discovery,
            &server,
            &env,
            true,
            &mut |url, opened| {
                let _ = url_tx.send(McpLoginUpdate::Url(url.to_string(), opened));
            },
            &|| flag.load(std::sync::atomic::Ordering::Relaxed),
        );
        let _ = tx.send(McpLoginUpdate::Done(result));
    });
    ask.mcp_login = Some(McpLogin {
        name: name.to_string(),
        cancel,
        updates: rx,
    });
...
/// Progress of a background `/mcps auth`.
fn poll_mcp_login(ask: &mut Interact, hint: &mut String, last_error: &mut String) {
    let Some(login) = ask.mcp_login.as_ref() else {
        return;
    };
    let name = login.name.clone();
    while let Ok(update) = login.updates.try_recv() {
        match update {
            McpLoginUpdate::Url(url, opened) => {
                *hint = if opened {
                    format!(
                        "Signing in to MCP server '{name}' in the browser. If it did not open, visit: {url}"
                    )
                } else {
                    format!("Sign in to MCP server '{name}': open {url}")
                };
            }
            McpLoginUpdate::Done(result) => {
                ask.mcp_login = None;
                match result {
                    Ok(message) => {
                        *hint = format!("{message} Restarting MCP servers…");
                        ask.mcp_restart = Some((name, message));
                    }
                    Err(error) => *last_error = format!("MCP sign-in failed: {error}"),
                }
                return;
            }
        }
...
/// Restart every MCP server by resuming in a fresh dsh with a rewritten
/// plan (dsh mounts servers when a session starts).
#[allow(clippy::too_many_arguments)]
fn restart_mcp_servers(
    launch: &Launch,
    effective: &mut config::EffectiveConfig,
    extra_env: &mut Vec<(String, String)>,
    patch: &mut Option<PathBuf>,
...
/// `/copy [N] [path]`: copy (or write) the Nth-latest non-empty response.
fn copy_response_command(args: &str, turns: &[Turn], grok_home: &Path) -> Result<String, String> {
    let words: Vec<&str> = args.split_whitespace().collect();
    let (nth, path_words) = match words.first().and_then(|word| word.parse::<usize>().ok()) {
        Some(0) => {
            return Err("usage: /copy [N] [path] (N counts from 1 = latest response)".into());
        }
        Some(nth) => (nth, &words[1..]),
        None => (1, &words[..]),
    };
    let responses: Vec<&Turn> = turns
        .iter()
        .rev()
        .filter(|turn| !turn.answer.trim().is_empty())
        .collect();
    let Some(turn) = responses.get(nth - 1) else {
        return Err(if responses.is_empty() {
            "/copy: there is no response to copy yet".into()
        } else {
            format!(
                "/copy: only {} response(s) in this session",
                responses.len()
            )
        });
    };
    let text = turn.answer.trim_end_matches('\n');
    if path_words.is_empty() {
        return Ok(clipboard::copy_live(text, grok_home).message);
    }
    let raw = path_words.join(" ");
    let path = clipboard::expand_tilde(&raw, &terminal_env::current_env());
    if let Some(parent) = path
        .parent()
        .filter(|parent| !parent.as_os_str().is_empty())
    {
        std::fs::create_dir_all(parent)
            .map_err(|error| format!("/copy: cannot create {}: {error}", parent.display()))?;
    }
    std::fs::write(&path, text.as_bytes())
        .map_err(|error| format!("/copy: cannot write {}: {error}", path.display()))?;
    Ok(format!("Wrote {} bytes to {}", text.len(), path.display()))
}

/// Ticket 168: MCP elicitations from the bridge.
fn apply_elicit_events(
    events: Vec<mcp_bridge::SurfaceEvent>,
    ask: &mut Interact,
    hint: &mut String,
) {
    use mcp_bridge::SurfaceEvent;
    for event in events {
        match event {
            SurfaceEvent::Opened(elicitation) => ask.elicit_waiting.push_back(elicitation),
            SurfaceEvent::Withdrawn(id) => {
                ask.elicit_waiting.retain(|waiting| waiting.id != id);
                if let Some(card) = ask.elicit.take_if(|card| card.id == id && !card.waiting) {
                    *hint = format!("MCP server '{}' withdrew its request", card.server);
                }
            }
            SurfaceEvent::Completed(id) => {
                if let Some(card) = ask.elicit.take_if(|card| card.id == id) {
                    *hint = format!("MCP server '{}' finished", card.server);
                }
            }
        }
    }
    if ask.elicit.is_none()
        && let Some(next) = ask.elicit_waiting.pop_front()
    {
        let card = elicit_ui::ElicitCard::new(&next);
        *hint = format!(
            "MCP server '{}' is asking you something; answer on the card",
            card.server
        );
        ask.elicit = Some(card);
    }
}

/// Keys for the MCP elicitation card. `true` when the key was used.
fn handle_elicit_key(
    key: crossterm::event::KeyEvent,
    ask: &mut Interact,
    composer: &PromptComposer,
    hint: &mut String,
) -> bool {
    let Some(card) = ask.elicit.as_mut() else {
        return false;
    };
    if card.parked {
    ...
    match card.handle_key(key) {
        elicit_ui::ElicitAction::Answer(answer) => {
            let verb = answer["action"].as_str().unwrap_or("").to_string();
            *hint = match ask.bridge.answer(&id, &answer) {
                Ok(()) => match verb.as_str() {
                    "accept" => "answer sent to the MCP server".into(),
                    "decline" => "declined; the MCP server was told no".into(),
                    _ => "cancelled the MCP request".into(),
                },
                Err(error) => format!("answer not delivered: {error}"),
            };
            close = true;
        }
        elicit_ui::ElicitAction::AcceptUrl(url) => {
            match ask
                .bridge
                .answer(&id, &serde_json::json!({"action": "accept"}))
            {
                Ok(()) => {
                    let opened = mcp_auth::open_browser(&url, &env);
                    *hint = if opened {
                        format!("opened {url}; o reopens, Esc dismisses")
                    } else {
                        format!("open {url} in your browser; Esc dismisses")
                    };
                }
                Err(error) => {
        ...
        elicit_ui::ElicitAction::Open(url) => {
            if !mcp_auth::open_browser(&url, &env) {
                *hint = format!("open {url} in your browser");
            }
        }
        elicit_ui::ElicitAction::Dismiss => close = true,
        elicit_ui::ElicitAction::Hint(text) => *hint = text,
        elicit_ui::ElicitAction::None => {}
    }
    if close {
        ask.elicit = None;
        apply_elicit_events(Vec::new(), ask, hint);
    }
    true
}
...
fn copy_interaction_text(text: &str, home: &Path) -> String {
    clipboard::copy_live(text, home).message
}
...
fn handle_interaction_key(
    ...
    if side.ask.card.is_none() && side.ask.review.is_none() && side.ask.elicit.is_some() {
        return (handle_elicit_key(key, &mut side.ask, composer, hint), None);
    }
...
fn apply_control_events(
    ...
    for event in events {
        match event {
            ...
            ControlEvent::MemoryModel { id, outcome } => side.memory.accept(id, outcome),
            ControlEvent::Usage { id, outcome } => {
                if let Some(text) = side.usage.accept(&id, outcome.map(|ledger| *ledger)) {
                    *hint = text;
                }
            }
            ...
            ControlEvent::Closed(reason) => {
                ...
                let pending: Vec<String> = side
                    ...
                    .collect();
                for id in pending {
                    if let Some(text) = side.usage.accept(&id, Err(reason.clone())) {
                        *hint = text;
                    }
                }
...
fn run_plain_turn(
    ...
    // Nor to deliver a monitor event to: no monitor tool either.
    extra_env.retain(|(key, _)| key != "CODSH_MONITOR");
    ...
    if launch.memory_flush && stop.code == 0 {
        match plain_memory_flush(&mut connection.client, &effective, interrupt, terminate) {
            Ok(notice) => eprintln!("{notice}"),
            Err(message) => {
                stop = PlainStop {
                    code: 1,
                    message: format!("memory flush skipped: {message}"),
                };
            }
        }
    }
    if launch.output_format != headless::OutputFormat::Plain
        && let Some(summary) = plain_usage(
...
/// children included (ticket 65). None (reported as `usage_absent`) for a
/// remote session, a control channel that never connected, a fold error, or
/// no answer in time.
fn plain_usage(
    client: &mut AcpClient,
    since_ms: u64,
    ...
    let id = format!("usage-plain-{since_ms}");
    client
        .send_control(&usage::request_message(&id, &session, Some(since_ms)))
        .ok()?;
    ...
    while Instant::now() < deadline {
        ...
        for event in client.poll_control() {
            if let control::ControlEvent::Usage { id: got, outcome } = event
                && got == id
            {
                return outcome.ok().and_then(|ledger| ledger.prompt);
            }
        }
    }
    None
...
/// `--memory-flush` (ticket 186): one flush of the finished -p turn through
/// the control plugin. Only a written session log is success; nothing to
/// store, a rejected answer, or a failure is the error text.
fn plain_memory_flush(
    client: &mut AcpClient,
    effective: &config::EffectiveConfig,
    interrupt: &AtomicBool,
    terminate: &AtomicBool,
    ...
    if client.remote {
        return Err("local memory is not available in a remote session".into());
    }
    let store =
        memory::open_store(&effective.grok_home, &effective.cwd).map_err(|error| error.message)?;
    let config = &effective.memory_capture;
    let active = memory_session_enabled(effective, None);
    if let Some(warning) = memory_capture::probe_log(&config.log) {
        eprintln!("{warning}");
    }
    let mut runner = memory_capture::Runner::default();
    ...
    let session = client.session_id.clone();
    {
        let send = memory_control_send(Some(client));
        ...
        runner.start_flush(&ctx, "user_requested")?;
    }
    loop {
        if plain_signal(interrupt, terminate) != 0 {
            let send = memory_control_send(Some(client));
            let _ = runner.cancel(Some(&store), &config.log, Some(&send));
            return Err("Memory flush cancelled.".into());
        }
        let _ = client.pump(Duration::from_millis(50));
        for event in client.poll_control() {
            if let control::ControlEvent::MemoryModel { id, outcome } = event {
                runner.accept(id, outcome);
            }
        }
        let send = memory_control_send(Some(client));
        let ctx = memory_capture::Ctx {
        ...
        if let Some(notice) = runner.settle(&ctx).pop() {
            return if notice.starts_with("Memory flushed:") {
                Ok(notice)
            } else {
                Err(notice)
            };
...
fn run() -> io::Result<()> {
    ...
    match &mode {
        ...
        LaunchMode::ImportSessions(flags) => {
            if flags.help {
                println!("{}", session_migrate::help());
                return Ok(());
            }
            return run_import_sessions(flags);
        }
        ...
        LaunchMode::UsageHelp => {
            println!("{}", usage::command_help());
            return Ok(());
        }
        LaunchMode::Usage { session_id, turn } => {
            let loaded = load_runtime_config(&launch);
            match usage::run_command(&loaded.dsh_home, session_id, *turn) {
                Ok(text) => {
                    println!("{text}");
                    return Ok(());
                }
        ...
        LaunchMode::Doctor(cli) => {
            let loaded = load_runtime_config(&launch);
            let ctx = clipboard::CopyContext::detect(&loaded.grok_home);
            let policy = notifications::resolve(&loaded.merged_table);
            let code = doctor::run_cli(cli, &ctx, &notifications::describe(&policy, &ctx.term));
            if code != 0 {
                std::process::exit(code);
            }
            return Ok(());
        ...
        _ if launch.memory_flush => {
            return Err(io::Error::other(
                "--memory-flush without a prompt requires --resume/-r or --continue/-c; this client flushes after a -p prompt only",
            ));
        }
        _ => {}
    }
    ...
    // Ticket 155: focus-gated notifications and the one-time SSH tip.
    let term_env = terminal_env::TermEnv::detect();
    let mut notifier = notifications::Notifier::new(
        notifications::resolve(&effective.merged_table),
        &term_env,
        terminal_env::current_env(),
    );
    let mut notified_session: Option<String> = None;
    if let Some(tip) = ssh_wrap_tip(&term_env, &effective) {
        hint = tip;
    }
    while !stopping.load(Ordering::Relaxed) {
        let was_compacting = compacting;
        let was_inflight = inflight;
        let had_permission = turns.last().is_some_and(|turn| turn.permission.is_some());
        ...
        notify_transitions(
            &mut notifier,
            &mut notified_session,
            client.as_ref().and_then(|active| active.session_id.clone()),
            &turns,
            had_permission,
            was_inflight && !inflight && !was_compacting && !compacting,
        );
        ...
        if live_board_session != board.session {
            ...
            let ended: usize = board
                ...
                .sum();
            if ended > 0 {
                hint = background::ended_hint(
                    ended - monitors.min(ended),
                    monitors,
        ...
        if let Some(detail) = disconnect {
            ...
            wake = None;
            let monitors = board.jobs.running_monitors();
        ...
        if let Some(dir) = client.as_ref().and_then(AcpClient::mcp_run_dir) {
            side.ask.bridge.watch(&dir, "tui");
        }
        poll_mcp_login(&mut side.ask, &mut hint, &mut last_error);
        if !inflight
            && !compacting
            && let Some((_, message)) = side.ask.mcp_restart.take()
        {
            match restart_mcp_servers(
                &launch,
                &mut effective,
                &mut extra_env,
                &mut patch,
                ...
                // One line: a tall notice here hid the next permission prompt.
                Ok(()) => hint = format!("{message} MCP servers restarted; /mcps lists them."),
                Err(error) => last_error = error,
            }
        }
        let bridge_events = side.ask.bridge.poll();
        if !bridge_events.is_empty()
            || (side.ask.elicit.is_none() && !side.ask.elicit_waiting.is_empty())
        {
            apply_elicit_events(bridge_events, &mut side.ask, &mut hint);
        }
        ...
        side.usage.sync(client.as_ref(), inflight);
        ...
        let shown_hint = match &overlay {
            ...
            Overlay::Plugins(plugin_overlay) => {
                let env = std::env::vars().collect();
                let mut snapshot = plugin::inspect(
                    &effective.grok_home,
                    &effective.cwd,
                    &env,
                    effective.workspace_trusted,
                );
                // Plugin MCP rows say what the live session actually mounted.
                let live = client
                    .as_ref()
                    .and_then(|active| {
                        mcp::live_rows(
                    ...
                    .map(|(rows, _)| rows)
                    .unwrap_or_default();
                plugin::annotate_live_mcp(&mut snapshot, &live);
        ...
        .with_usage(side.usage.status.clone());
        let status_key = format!(
            ...
            inflight,
            screen.as_str(),
            status_snap.usage.as_ref().map(|usage| (
                usage.model_calls,
        ...
        let view = StatusView {
            ...
            approval: image_approval_line(&turns),
            cancelling,
            hint: &shown_hint,
            resumed,
        ...
        match event::read()? {
            ...
            Event::Key(key) if key.kind == KeyEventKind::Press => {
                    ...
                    && matches!(nav.overlay, NavOverlay::None)
                {
                    ...
                    if key.code == KeyCode::BackTab && !side.ask.open() && side.ask.elicit.is_none()
                    {
                    ...
                    match nav.handle_key_with_prompt(
                        ...
                        NavCommand::CopyBlock => {
                            if nav.focus == Focus::Scrollback && nav.copy_target().is_none() {
                                ...
                                hint = clipboard::copy_live(&text, &effective.grok_home).message;
                            }
                            continue;
                        }
                            ...
                            plugin::OverlayAction::Run(command) => {
                                match plugin::run(
                                    ...
                                    Ok(message) => {
                                        hint = message;
                                        last_error.clear();
                                        // A withdrawn plugin MCP server's remembered
                                        // approvals were forgotten on disk; the live
                                        // policy file dsh reads per call follows now.
                                        let kept = permission::load_grants(
                                            &effective.permission.grants_path,
                                        )
                                        .allowed_mcp;
                                        let allowed = &mut effective.permission.grants.allowed_mcp;
                                        let before = allowed.len();
                                        allowed.retain(|tool| kept.contains(tool));
                                        if allowed.len() != before {
                                            let _ = permission::write_policy_file(
                                                &effective.dsh_home,
            ...
            Event::FocusGained => notifier.focus(true, Instant::now()),
            Event::FocusLost => notifier.focus(false, Instant::now()),
...
const DEFAULT_EXIT_TIMEOUT_SECS: u64 = 20;

/// `None` disables the watchdog (`0`); invalid values use the default.
fn exit_timeout(value: Option<&str>) -> Option<Duration> {
    match value.map(str::trim).filter(|value| !value.is_empty()) {
        None => Some(Duration::from_secs(DEFAULT_EXIT_TIMEOUT_SECS)),
        Some(value) => match value.parse::<u64>() {
            Ok(0) => None,
            Ok(secs) => Some(Duration::from_secs(secs)),
            Err(_) => Some(Duration::from_secs(DEFAULT_EXIT_TIMEOUT_SECS)),
        },
    }
}

fn arm_exit_watchdog(value: Option<&str>) {
    let Some(timeout) = exit_timeout(value) else {
        return;
    };
    std::thread::spawn(move || {
        std::thread::sleep(timeout);
        // If restoring the terminal itself hangs, a hard exit follows 5s later.
        std::thread::spawn(|| {
        ...
        restore_terminal();
        eprintln!(
            "codsh: shutdown took longer than {}s; exiting (GROK_EXIT_TIMEOUT_SECS, 0 disables)",
            timeout.as_secs()
        );
...
/// Fire notification events for what changed in this loop iteration.
fn notify_transitions(
    notifier: &mut notifications::Notifier,
    notified_session: &mut Option<String>,
    session: Option<String>,
    turns: &[Turn],
    had_permission: bool,
    turn_ended: bool,
    ...
    if session.is_some() && *notified_session != session {
        notifier.set_session(session.as_deref());
        if notified_session.is_none() {
            notifier.event(
                notifications::Event::SessionReady,
                "Session ready",
                now,
                &mut stdout,
            );
        }
        *notified_session = session;
    }
    let last = turns.last();
    if !had_permission && last.is_some_and(|turn| turn.permission.is_some()) {
        notifier.event(
            notifications::Event::ApprovalRequired,
            "Approval required",
            now,
            &mut stdout,
        );
    }
    if turn_ended && let Some(turn) = last {
        if let Some(error) = turn.error.as_ref().filter(|_| !turn.cancelled) {
            let first = error.lines().next().unwrap_or("error");
            let message = format!(
                "Agent error: {}",
                first.chars().take(120).collect::<String>()
            );
            notifier.event(notifications::Event::AgentError, &message, now, &mut stdout);
        } else if !turn.cancelled {
            notifier.event(
                notifications::Event::TurnComplete,
                "Turn complete",
                now,
                &mut stdout,
            );
    ...
    notifier.tick(now, &mut stdout);
}

/// One-time tip for SSH sessions not started through `codsh wrap`.
/// `[ui.contextual_hints] ssh_wrap = false` turns it off.
fn ssh_wrap_tip(
    term: &terminal_env::TermEnv,
    effective: &config::EffectiveConfig,
) -> Option<String> {
    ...
    let enabled = effective
        .merged_table
        .get("ui")
        .and_then(|ui| ui.get("contextual_hints"))
        .and_then(|hints| hints.get("ssh_wrap"))
        .and_then(|value| value.as_bool())
        .unwrap_or(true);
    if !enabled {
        return None;
    }
    let marker = effective.grok_home.join("hints").join("ssh_wrap.shown");
    if marker.exists() {
        return None;
    }
    ...
    Some(
        "SSH session: copies travel as OSC 52 through your terminal. Run /doctor for details and fixes (or start from your machine with `codsh --rust wrap ssh <host>`)."
            .into(),
    )
}
...
fn open_notice_turn(
        ...
        && let Some(turn) = turns.last_mut().filter(|turn| turn_is_empty(turn))
    {
        turn.user = line;
        open.generic = false;
    } else if background::is_notice_line(&line)
        && let Some(turn) = turns.last_mut().filter(|turn| {
            !turn.done && turn_is_empty(turn) && background::is_notice_line(&turn.user)
        })
    {
        // Messages dsh handed over at the same step boundary (a monitor
        // event and its command's exit) share one turn.
        turn.user.push('\n');
        turn.user.push_str(&line);
    } else {
...
fn apply_event_stream(
    ...
    for event in events {
        ...
        match board.jobs.apply(&event) {
            ...
            Some(background::Signal::Monitor { session, summary }) if ours(&session) => {
                let line = background::monitor_line(&summary);
                open_notice_turn(turns, inflight, wake, hint, compacting, line);
            }
            Some(_) => {}
        }
...
/// Ticket 187: a running image_gen / image_edit row shows the service, the
/// elapsed time, and how to cancel. The final result replaces that line.
/// The status line of a pending image approval: what is sent where, and cost.
fn image_approval_line(turns: &[Turn]) -> Option<String> {
    let turn = turns.last()?;
    let permission = turn.permission.as_ref()?;
    let tool = turn
        .tools
        .iter()
        .find(|tool| tool.id == permission.tool_call_id)?;
...
fn decorate_image_blocks(
    turns: &mut [Turn],
    started: &mut std::collections::HashMap<String, Instant>,
    services: &image_gen::ImageServices,
) {
    for turn in turns.iter_mut() {
        for tool in &mut turn.tools {
            if let Some(title) = image_gen::card_title(&tool.title, &tool.raw_input, services) {
                tool.title = title;
            }
        }
    }
    for turn in turns.iter_mut().rev().take(2) {
        let waiting = turn.permission.as_ref().map(|p| p.tool_call_id.clone());
        for tool in &mut turn.tools {
            if !image_gen::is_image_title(&tool.title) {
                continue;
            }
            if tool.status == "in_progress" && waiting.as_deref() != Some(tool.id.as_str()) {
                let since = *started.entry(tool.id.clone()).or_insert_with(Instant::now);
                let line = image_gen::progress_line(&tool.title, since.elapsed());
                if tool.result.is_empty() || tool.result.starts_with("waiting for ") {
                    tool.result = line;
                }
            } else if tool.result.starts_with("waiting for ") {
                if tool.status != "pending" {
                    tool.result.clear();
                }
                started.remove(&tool.id);
...
fn decorate_subagent_blocks(turns: &mut [Turn], board: &subagents::Board) {
    if board.entries.is_empty() && board.workflows.is_empty() && board.messages.is_empty() {
        return;
    }
    for turn in turns.iter_mut().rev() {
        for tool in &mut turn.tools {
            let title = if let Some(entry) = board.get(&tool.id) {
                ...
                run.block_title()
            } else if let Some(row) = board.message(&tool.id) {
                // Ticket 173: the row names the target and the outcome; the
                // expanded block shows the delivery, the text, and the result.
                let details = row.details();
                if tool.result != details {
                    tool.result = details;
                }
                row.title()
            } else {
...
fn saved_workflow_slash(
    ...
    // A name that is defined but not runnable (invalid file, ambiguous, its
    // plugin disabled) still goes to the run manager, whose reply says why
    // it cannot start.
...
/// Ticket 186: the memory store for this workspace, opened once per cwd.
fn memory_capture_store(
    side: &mut Intervene,
    effective: &config::EffectiveConfig,
) -> Option<memory::Store> {
    let fresh = side
        .memory_store
        .as_ref()
        .is_none_or(|(home, cwd, _)| home != &effective.grok_home || cwd != &effective.cwd);
    if fresh {
        side.memory_store = memory::open_store(&effective.grok_home, &effective.cwd)
            .ok()
            .map(|store| (effective.grok_home.clone(), effective.cwd.clone(), store));
    }
    side.memory_store
        .as_ref()
...
/// Sends one memory control message to the local dsh of this session.
fn memory_control_send(client: Option<&AcpClient>) -> impl Fn(&Value) -> Result<(), String> + '_ {
    move |message| match client {
        Some(active) if active.remote => {
            Err("local memory is not available in a remote session".into())
        }
        Some(active) if active.control_ready() => active.send_control(message),
        Some(active) => Err(active.control_unavailable()),
        None => Err("no live dsh session; send a prompt first".into()),
    }
}

/// Keep the session-end target current, apply memory answers, cancel a
/// request whose session is gone (`/new`, a switch, a disconnect), and run
/// the idle flush and automatic Dream timers.
#[allow(clippy::too_many_arguments)]
fn sync_memory_capture(
    side: &mut Intervene,
    client: &mut Option<AcpClient>,
    effective: &config::EffectiveConfig,
    memory_session_on: Option<bool>,
    ...
    let Some(store) = memory_capture_store(side, effective) else {
        return;
    };
    let config = &effective.memory_capture;
    let enabled = memory_session_enabled(effective, memory_session_on);
    if enabled && !side.memory_notices_shown {
        side.memory_notices_shown = true;
        if let Some(warning) = memory_capture::probe_log(&config.log) {
            *last_error = warning;
        } else if config.v2_requested {
            *hint = memory_capture::V2_REFUSAL.into();
        }
    }
    if let Some(active) = client.as_mut().filter(|active| !active.remote) {
        let target_active = enabled && !config.v2_requested;
        let stale = active.memory_target.as_ref().is_none_or(|target| {
            target.active != target_active
                || target.save_on_end != config.save_on_end
                || target.store.workspace_dir != store.workspace_dir
                || target.log != config.log
        });
        if stale {
            active.memory_target = Some(memory_capture::CaptureTarget {
                store: store.clone(),
                save_on_end: config.save_on_end,
    ...
    let session_id = client
        ...
        .filter(|active| !active.remote)
        .and_then(|active| active.session_id.clone());
    let send = memory_control_send(client.as_ref());
    if side
        .memory
        .job_session()
...
fn dispatch_memory_command(
    ...
    let Some(store) = memory_capture_store(side, effective) else {
        *last_error = format!("{command}: the local memory store could not be opened");
        return;
    };
    let session_id = client
        .as_ref()
    ...
    let ctx = memory_capture::Ctx {
        store: &store,
        config: &effective.memory_capture,
        active: memory_session_enabled(effective, memory_session_on),
        session_id: session_id.as_deref(),
        send: &send,
    };
    let result = if command == "/flush" {
        side.memory.start_flush(&ctx, "user_requested").map(Some)
    } else {
        side.memory.start_dream(&ctx, true)
    };
    match result {
        Ok(Some(text)) => {
            *hint = text;
...
fn dispatch_composer_command(
    ...
    // `/imagine <description>` (ticket 187): the model calls image_gen with
    // the prompt verbatim. The transcript shows what was typed.
    let imagine = image_gen::imagine_args(text);
    if let Some(args) = imagine {
        if args.is_empty() {
            composer.restore_slash_draft();
            *hint = image_gen::IMAGINE_USAGE.into();
            last_error.clear();
            return Ok(());
        }
        if !effective.images.gen_enabled {
            composer.restore_slash_draft();
    ...
    if text.trim() == "/images" || text.trim().starts_with("/images ") {
        composer.restore_slash_draft();
        last_error.clear();
        match images_command(
            text.trim().trim_start_matches("/images").trim(),
            client.as_ref(),
            effective,
        ) {
            Ok(message) => *hint = message,
    ...
    // `/copy [N] [path]` (ticket 155): the Nth-latest response's source markdown.
    if text.trim() == "/copy" || text.trim().starts_with("/copy ") {
        composer.restore_slash_draft();
        last_error.clear();
        match copy_response_command(
            text.trim().trim_start_matches("/copy"),
            turns,
            &effective.grok_home,
        ) {
            Ok(message) => *hint = message,
    ...
    // `/doctor` and its aliases (ticket 155): terminal diagnostics and tmux fixes.
    if let Some(command @ ("/doctor" | "/terminal-setup" | "/terminal-check" | "/terminal-info")) =
        text.split_whitespace().next()
    {
        composer.restore_slash_draft();
        last_error.clear();
        let ctx = clipboard::CopyContext::detect(&effective.grok_home);
        let described =
            notifications::describe(&notifications::resolve(&effective.merged_table), &ctx.term);
        *hint = doctor::slash(text.trim().trim_start_matches(command), &ctx, &described);
        return Ok(());
    }
    if text.trim() == "/queue" {
        composer.restore_slash_draft();
    ...
    // `/usage` (alias `/cost`, ticket 65): the session usage ledger.
    if matches!(text.trim(), "/usage" | "/cost") {
        composer.restore_slash_draft();
        last_error.clear();
        match client.as_ref() {
            None => *hint = usage::unavailable_text(""),
            Some(active) => match side.usage.request(active, UsagePurpose::Show) {
                Ok(()) => *hint = "Reading session usage…".into(),
                Err(error) => *hint = usage::unavailable_text(&error),
            },
        }
        return Ok(());
    }
    ...
    if let Some(command @ ("/flush" | "/dream")) = text.split_whitespace().next() {
        composer.restore_slash_draft();
        if text.trim() != command {
            *last_error = format!("usage: {command} (it takes no arguments)");
            return Ok(());
        }
    ...
    if let Some(action) = mcp::parse_slash(trimmed) {
        ...
        if let mcp::Slash::Auth(name) = &action {
            let discovery = mcp::discover(&input);
            match discovery.get(name).map(|entry| &entry.def.transport) {
                None => *last_error = format!("No MCP server named '{name}'."),
                Some(mcp::Transport::Stdio { .. }) => {
                    *last_error = format!(
                        "MCP server '{name}' is a stdio server; OAuth applies to remote servers."
                    );
                }
                Some(_) => {
                    start_mcp_login(&mut side.ask, discovery, name);
                    *hint = format!("Signing in to MCP server '{name}'…");
                    last_error.clear();
                }
            }
            return Ok(());
        }
        if let mcp::Slash::Logout(name) = &action {
            match mcp::logout(&mcp::discover(&input), name, &env) {
                Ok(message) => {
                    *hint = message.clone();
                    last_error.clear();
                    side.ask.mcp_restart = Some((name.clone(), message));
                }
                Err(error) => *last_error = error,
            }
            return Ok(());
...
/// `/images` lists this session's saved images; `/images open [N]` opens one
/// with the system viewer (CODSH_IMAGE_OPENER, `open`, or `xdg-open`).
fn images_command(
    args: &str,
    client: Option<&AcpClient>,
    effective: &config::EffectiveConfig,
    ...
    let session = client
        .and_then(|active| active.session_id.as_deref())
        .ok_or("no session yet; no image has been saved")?;
    let folder = interaction::plan_root(&effective.grok_home, &effective.cwd).join(session);
    let mut words = args.split_whitespace();
    match words.next() {
        None | Some("list") => Ok(image_gen::listing(&folder)),
        Some("open") => {
            let image = image_gen::find_image(&folder, words.next().unwrap_or(""))?;
            let env: std::collections::BTreeMap<String, String> = std::env::vars().collect();
            let program = image_gen::open_image(&image.path, &env)?;
            Ok(format!("Opened {} with {program}", image.summary()))
        }
...
fn submit_composer_prompt(
    ...
    if let Some(active) = client.as_mut() {
        ...
        match active.submit_prompt_blocks(&blocks) {
            Ok(_) => {
                active.note_memory_query(&transcript);
                turns.push(Turn {
...
fn run_mcp(invocation: &mcp::McpInvocation, loaded: &config::EffectiveConfig) -> io::Result<i32> {
    ...
    let (label, outcome) = match &invocation.command {
        ...
        mcp::McpCommand::Login { name } => ("login", mcp::run_login(&input, name)),
        mcp::McpCommand::Logout { name } => ("logout", mcp::run_logout(&input, name)),
    };
...
fn main() {
    ...
    // It must not touch the terminal, config, or sandbox setup.
    {
        ...
        if raw.first().map(String::as_str) == Some(mcp_remote::SUBCOMMAND) {
            std::process::exit(mcp_remote::run_proxy(&raw[1..]));
        }
        ...
        // `wrap` owns every word after it (`wrap ssh -p 22 host`) and runs
        // with the user's own environment; it restores its own terminal state.
        if raw.first().map(String::as_str) == Some("wrap") {
            match raw.get(1).map(String::as_str) {
                None => {
                    eprintln!("{}", wrap::usage());
                    std::process::exit(2);
                }
                Some("--help" | "-h") if raw.len() == 2 => {
                    println!("{}", wrap::usage());
                    std::process::exit(0);
                }
                _ => std::process::exit(wrap::run(&raw[1..], &wrap::grok_home())),
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn exit_timeout_parses_default_zero_and_garbage() {
        assert_eq!(exit_timeout(None), Some(Duration::from_secs(20)));
        assert_eq!(exit_timeout(Some("0")), None);
        assert_eq!(exit_timeout(Some("3")), Some(Duration::from_secs(3)));
        assert_eq!(exit_timeout(Some("soon")), Some(Duration::from_secs(20)));
    }

    #[test]
    fn copy_command_picks_nth_response_and_writes_files() {
        let dir = tempfile::tempdir().unwrap();
        let mut first = live_turn("one");
        first.answer = "FIRST **md**\n".into();
        let mut empty = live_turn("two");
        empty.answer = "  ".into();
        let mut last = live_turn("three");
        last.answer = "LAST".into();
        let turns = vec![first, empty, last];
        let target = dir.path().join("out/copy.md");
        let written =
            copy_response_command(&format!(" 2 {}", target.display()), &turns, dir.path()).unwrap();
        assert!(written.contains("Wrote 12 bytes"), "{written}");
        assert_eq!(std::fs::read_to_string(&target).unwrap(), "FIRST **md**");
        assert!(
            copy_response_command(" 3", &turns, dir.path())
                .unwrap_err()
                .contains("only 2")
        );
        assert!(copy_response_command(" 0", &turns, dir.path()).is_err());
        assert!(
            copy_response_command("", &[], dir.path())
                .unwrap_err()
    ...
    #[test]
    fn notices_claimed_at_one_step_share_a_turn() {
        let mut turns = vec![background_turn("MON_CALL probe".into())];
        let mut inflight = true;
        let mut wake = None;
        let mut hint = String::new();
        let event = background::monitor_line("pty once: OUT");
        open_notice_turn(
            &mut turns,
            &mut inflight,
            &mut wake,
        ...
        assert!(turns[0].done);
        let exit = background::notice_line("monitor [monitor] pty once [status: completed]");
        open_notice_turn(&mut turns, &mut inflight, &mut wake, &mut hint, false, exit);
        assert_eq!(turns.len(), 2);
        assert_eq!(
            turns[1].user,
            "◎ Monitor event · pty once: OUT\n◎ Task completed · monitor [monitor] pty once [status: completed]"
        );
        // Once the model answered, the next notice opens its own turn.
        turns[1].answer = "RUST_MON_EVENT".into();
        let next = background::monitor_line("pty once: AGAIN");
        open_notice_turn(&mut turns, &mut inflight, &mut wake, &mut hint, false, next);
        assert_eq!(turns.len(), 3);
        assert!(turns[1].done);
        assert_eq!(turns[2].user, "◎ Monitor event · pty once: AGAIN");
    }
    ...
    fn loop_lines_and_the_scheduler_switch() {
        ...
        assert!(!acp::INHERITED_ENV.contains(&"CODSH_MONITOR"));
        assert!(acp::INHERITED_ENV.contains(&"CODSH_TEST_MONITOR_TIME_SCALE"));
    ...
    fn integrated_sandbox_memory_web_and_cwd_flags_are_not_refused() {
        ...
        assert!(later.to_string().contains("not implemented"), "{later}");
        let flush = parse_launch(&args(&["-p", "hello", "--memory-flush"])).unwrap();
        assert!(flush.memory_flush);
    }
