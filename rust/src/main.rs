...
fn run() -> io::Result<()> {
    ...
    while !stopping.load(Ordering::Relaxed) {
        ...
        if live_board_session != board.session {
            ...
            // Scheduled loops live in one dsh process and session: a switch
            // or a restarted dsh process ended them, and the user is told.
            let loops = board.schedules.end_all();
            if loops > 0 {
                hint = format!(
                    "{loops} scheduled loop(s) ended with the previous dsh session; loops are session-only"
                );
            }
        ...
        if let Some(detail) = disconnect {
            ...
            let loops = board.schedules.end_all();
            if loops > 0 {
                hint = format!("{loops} scheduled loop(s) ended when dsh exited");
            }
        }
