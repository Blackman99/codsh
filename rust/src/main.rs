                ...
                if let Some(path) = action.strip_prefix("copy ") {
                    ...
                    *hint = if browser.session_enabled {
                        if browser.turn_sent {
                            "memory on from the next new session".into()
                        } else {
                    ...
                    *overlay = Overlay::None;
                    ...
                    || action.starts_with("memory on from the next new session")
...
fn dispatch_composer_command(
    ...
    if let Some(action) = memory_slash(trimmed) {
        ...
        match action {
            MemorySlash::Browse => {
                match open_memory_browser(overlay, effective, *memory_session_on, !turns.is_empty())
                {
                    Ok(()) => {
                        hint.clear();
                        last_error.clear();
