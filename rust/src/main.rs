...
fn apply_events(
    ...
    for event in events {
        match event {
            ...
            // The board owns lifecycle lines; `absorb_subagent_events` runs first.
            AcpEvent::ConfigOptions { .. } | AcpEvent::Subagent { .. } => {}
        }
    }
    disconnect
}
...
fn run() -> io::Result<()> {
    ...
    while !stopping.load(Ordering::Relaxed) {
        ...
        let disconnect = client.as_mut().and_then(|active| {
            let events = absorb_subagent_events(&mut board, active.pump(Duration::ZERO), &mut hint);
            apply_events(
                &mut turns,
                &mut inflight,
                &mut meter,
        ...
        match event::read()? {
            ...
            Event::Key(key) if key.kind == KeyEventKind::Press => {
                ...
                if let Some(modal) = tasks.as_mut() {
                    if handle_tasks_key(key, modal, &mut board, &effective.dsh_home) {
                        tasks = None;
                    }
                    continue;
                }
...
fn decorate_subagent_blocks(turns: &mut [Turn], board: &subagents::Board) {
    ...
    for turn in turns.iter_mut().rev() {
        for tool in &mut turn.tools {
            if let Some(entry) = board.get(&tool.id) {
                let title = entry.block_title();
                if tool.title != title {
                    tool.title = title;
                }
            }
