...
fn status_line(view: StatusView<'_>) -> String {
    ...
    let header = format!(
        "mode={} | {}",
        screen.as_str(),
        filesystem_sandbox::status_line(filesystem_sandbox::active())
    );
...
fn run() -> io::Result<()> {
    ...
    if let Some(script) = &launch.sandbox_probe {
        ...
        command.arg(script);
        // The probe is the shell child the profile describes. A filtered
        // environment replaces the inherited one; an unfiltered profile
        // still sees the launch environment.
        if let Some(filtered) = filesystem_sandbox::active().and_then(|item| item.shell_env.clone())
        {
            command.env_clear();
