...
2. **Spec (Gate 1)** —— 自动合成（to-spec）。你确认。记录分支、基底 Commit 与精确验收命令。
3. **Tickets 与基线 (Gate 2)** —— 自动切成可独立验证的垂直切片并注入发版合规任务。你批准。先跑业务与仓库全局基线。
4. **落地** —— 机械化 TDD 红绿取证；单 Ticket 3 轮修错熔断；续跑级联重验；每个变绿的 ticket 产生单次全绿提交。较大的计划以 fresh agent 的 Ralph 循环执行；一轮进行中，工作行显示这一轮及其最近一次调用（计划行已经钉在 chrome 里，不再叠一份进度），spec 里的勾选一变、计划行随即更新，轮次结束的那一行写明做了多少事——Esc 会中断整个循环，哪怕 ticket 还没做完。
5. **完成 (双层 DoD)** —— 验收命令实跑 Exit Code 0 且仓库全局零新增报错；弹出合流选择（`ship · deliver`：合并、提 PR、保留分支）。

裸 `/ship` 会对已有进度级联重验后继续续跑未完成的 spec。
