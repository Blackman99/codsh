...
选择器。查询以 `!` 开头之前，点文件和 `.gitignore` 匹配项保持隐藏。Enter
或 Tab 把所选文件附成芯片；`:10-50` 保留该行范围，带空格的路径写成
`@"my file.rs"`。粘贴工作区路径等于拖入。芯片上按 Backspace 移除，Ctrl+Z
恢复。提交时才读取文件：已移除的芯片不会发送；文件缺失、超过 256 KiB、
没有读取权限，或预览后内容已变化，都留在输入框并给出明确提示，且不会带上
文件字节。dsh 和模型收到的是获准的文本。恢复会话时显示同一个 `@path`
引用。`chips=false` 只表示当前草稿没有附件。中文/组合字符、大段粘贴、缩放
