...
最小模式下 `/rewind` 与 `/fork` 会重置该原生缓冲，而不是把已丢弃回合追加进去。官方 `xai-grok-markdown` 渲染流式 Markdown、表格、代码、mermaid 节点、思考以及 dsh 工具卡/差异，会话会保留标题、代码、表格和差异的颜色。美化模式会隐藏行内 HTML 标签，并把 ZWJ 表情保持在同一个单元格；失败的工具用失败色显示 `failed` 和 `[error]`，而不是成功。Esc 关闭完整内容并恢复折叠后的记录。长结果会折叠；Tab 后按 `l`/`→` 展开，`r` 切换原文，Enter 打开完整内容，`y` 复制原文。最小模式下 `/expand` 会重印上一个折叠块；`/transcript`（`/log`）用 `$PAGER` 打开完整原文。
