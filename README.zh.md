...
- 本会话发起的子代理常驻在输入框下方计数——`subagents 2 · 1 running · 1 done · Ctrl+G`。Ctrl+G 或点这一行打开列表：每个子代理的状态、用时、调用数和最近一次调用；Enter 或点击进入其中一个，状态行会写明是哪一个。跑完的子代理仍留在列表里，可以只读打开。`/subagents` 打印同一份列表。
