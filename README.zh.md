...
- **原生视觉、纯文本模型都能粘贴图片**——Ctrl+V 直接挂进输入框；用 `/model deepseek-v4-flash-vision-exp` 选择 DeepSeek 实验性视觉路由后，图片作为一等附件发送。Flash、Pro 等纯文本路由仍会把图片存成 agent 可用工具打开的文件——配 `CODSH_VISION_*` 指向任意 OpenAI 兼容视觉端点时，一份逐字转写的描述随同一条消息送达。
