...
`codsh --rust -p "任务"`（或 `--single`、`--prompt-file <路径>`、`--prompt-json <内容块>`）通过同一个 dsh ACP 会话执行一次提示，stdout 只打印最终答案。思考、工具卡片和错误不会进入 stdout；诊断写到 stderr。`--cwd <路径>` 是交给 dsh 的工作目录。纯文本提示配合 `--continue` 或 `--resume <id>` 会继续该会话，`--fork-session` 会复制会话。`--max-turns <N>` 在 dsh 代理循环里、下一次模型步骤开始前取消。`--tools` 与 `--disallowed-tools` 把已注册的 dsh 工具名交给 dsh 的工具过滤；未知名称是错误，不会被静默跳过。`--allow`/`--deny` 仍然只控制执行，不会移除工具。位置参数不会进入纯文本模式，管道里的 stdin 也不是提示。`--output-format json`、代理选择、沙箱、子代理、计划、网页搜索、worktree 和记忆相关参数会明确失败，并留给后续任务。SIGINT 退出码为 130，SIGTERM 为 143，错误参数或 dsh 错误为 1。
