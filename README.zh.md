...
每次 `/ship` 只注入一份合同（grill / to-spec / to-tickets / TDD），避免后面阶段把正在执行的合同挤掉。MetaBar 芯片和计划行跟着磁盘上的 Status 与勾选走；`/ship` 返回后它们会撤下，下一句普通对话不再显示 ship 状态。裸 `/ship` 会对已有进度级联重验后继续续跑未完成的 spec。
...
- `/diff` 把未提交的改动送进同一个阅读器，而不是让它刷过去；展开装不下的工具组，点一下也在那里打开。管道里它依然只是若干行。
