...
Gate 1 Confirm freezes the Main Track — the idea, numbered Track-N decisions, and Out of Scope — compiles a sealed Mission Contract (`.scratch/<slug>/mission.contract.json`) the runner owns, and later turns are prepended with that snapshot plus a compact contract summary so landing cannot rewrite the design. After seal, Main Track rewrites are restored on disk; land injects only the active ticket. If an unrelated `/goal` is current, `/ship` pauses it and asks `ship · occupancy` (Replace / Abort) before any phase starts; on a pipe it auto-Replaces. `/goal` during a run shows the `[ship]` compass. Chrome is unchanged: no extra chrome, no GoalBar.
