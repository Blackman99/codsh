...
`/ship <one-sentence idea>` — pre-flight isolation, wayfinder, grill, two auto-confirmed gates, a Landing wave, dual-layer DoD. After every HITL answer the same invocation continues; `/goal` stays disarmed. The two-ring panorama (overlay, teaser, Web) is always on:

0. **Pre-flight & Isolation** — checks working tree hygiene (`ship · preflight` prompt if dirty); cuts an isolated `ship/<slug>` branch so the base branch stays pristine. Occupancy (`ship · occupancy`) still asks on a TTY when an unrelated `/goal` is current.
1. **Wayfinder** — before grill, name the destination and resolve the open decisions. Every run charts a named decision map and dependency-linked decision tickets on the configured tracker (local Markdown under `.scratch/<slug>/wayfinder/` otherwise); an empty inner ring is valid. These are decisions, not implementation tickets. Unfinished maps remain `Status: wayfinding`. The runner continues in the same invocation after each HITL answer, waking the parent for at most one unblocked non-research decision ticket at a time. A small, already-clear route records a confirmed no-map handoff and auto-advances to grill. The contract ships with codsh; no separate skill installation is required.
2. **Grill** — the grill-me skill: recon first, then a design-tree interview; each round batches the unblocked frontier with a recommended answer and `header` `ship · grill`; ←/→ revisit an earlier answer in that round; a write-in option is an inline field when focused. The card wraps the whole question — it never clips the body to two lines. Nothing is assumed until the frontier is empty and confirmed. Answering a round injects the next one in this invocation.
3. **Spec (Gate 1)** — synthesized automatically as the to-spec skill (exhaustive stories, public seams, Out of Scope). Keeps `## Original Requirement` as the user's wording, distinct from the compact Main Track. The runner auto-Confirms with a transcript notice; interrupt still aborts. After seal there is no Edit modal — a contradiction is a `## Blocker`. Records branch, base commit, decisions, proving commands, and a `.scratch/` copy (tracker if configured).
4. **Tickets & Baseline (Gate 2)** — to-tickets vertical slices with a DAG, per-ticket acceptance checklists, `.scratch/.../issues/` files, plus the release compliance ticket. The runner auto-Confirms the same way. Baseline runs across proof commands and repo guardrails.
5. **Landing** — the tdd skill: one red test witnessed failing, then minimal green, then the suite. A Landing wave dispatches every currently unblocked, unclaimed landing ticket in parallel worktrees; the parent serial-merges Ready-set (lowest `landing:N`) then proves. There is no landing turn-budget breaker; an unresolved `## Blocker` stops automatic continuation until archived. Cascading re-verification unticks only failed proofs and already-closed DAG dependents. Investigation, research, ticket implementation, and review go to a fresh `subagent` (not a history fork). The parent keeps questions, gates, and coordination, and independently re-runs final proofs. Children return at most 20 lines plus evidence/log paths. A runner-dispatched in-process child is a Child view Fold, not a fake `subagent` card. `/ship` does not call Ralph; the tool remains available for an explicit request outside the workflow. Esc stops coordination, including a running ticket.
6. **Done (Dual-Layer DoD)** — verifies acceptance criteria (exit 0) and zero new repo failures against baseline; delivery auto-picks Merge back (fast-forward when possible, else squash). Coverage is original requirement → Track-N → acceptance → ticket → evidence; a green suite cannot hide a missing requirement or an out-of-scope change.
