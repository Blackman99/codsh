...
- **Paste an image, natively or into a text-only model** — Ctrl+V attaches it; select DeepSeek's experimental vision route with `/model deepseek-v4-flash-vision-exp` to send first-class image attachments. Text-only routes such as Flash and Pro still get a file the agent can open with tools — and with `CODSH_VISION_*` pointing at any OpenAI-compatible vision endpoint, a verbatim description rides the same message.
