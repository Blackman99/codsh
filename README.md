...
1. **Grill** — the grill-me skill: recon first, then a design-tree interview; each round batches the unblocked frontier with a recommended answer and `header` `ship · grill`; ←/→ revisit an earlier answer in that round; a write-in option is an inline field when focused. Nothing is assumed until the frontier is empty and confirmed.
