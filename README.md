...
`codsh --rust -p "task"` (or `--single`, `--prompt-file <path>`, `--prompt-json <blocks>`) runs one prompt through the same dsh ACP session and prints only the final answer on stdout. Thoughts, tool cards, and errors stay off stdout; diagnostics go to stderr. `--cwd <path>` is the workspace dsh receives. `--continue` or `--resume <id>` with a plain prompt continues that session, and `--fork-session` copies it. `--max-turns <N>` cancels inside the dsh agent loop before the next model step. `--tools` and `--disallowed-tools` pass registered dsh tool names to dsh's tool filter; an unknown name is an error, not a silent skip. `--allow`/`--deny` still gate execution and do not remove the tool. A positional prompt does not start plain mode, and piped stdin is not the prompt. `--output-format json`, agent selection, sandbox, subagents, plan, web search, worktrees, and memory flags fail explicitly and remain later tickets. SIGINT exits 130, SIGTERM exits 143, and a bad flag or dsh error exits 1.
