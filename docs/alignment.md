...
| Prompt-top anchoring | Grok Build's page-flip behavior places a submitted prompt at the viewport top while reply rows consume the space beneath it | `aligned` | screen.spec anchor lifecycle/streaming/resize/manual cancellation/replay cases; experience e2e paced streaming capture. codsh models the gap as display-only tail rows: search, selection, folds, scrollback trimming, replay, and pipes never observe them |
| Conversation timeline | Grok Build renders a windowed turn rail with current ownership, overflow markers, hover preview, and click navigation | `aligned` | timeline.spec pure windowing; screen.spec reserved-column paint/hit/preview/selector-hide/trim/resize; experience e2e rail ownership across sticky turn boundaries. codsh uses its already-reserved anti-wrap column, so transcript width and chrome height stay unchanged |
...
| Update-available chrome | claude, opencode | `open` | rest surface; low confidence on exact shape |
