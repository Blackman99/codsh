...
3. **Enter and live streaming.**
   Enter still `sessions.get`s the id; missing still flashes that the subagent is no longer running. On hit: push the stack, clear the Viewport, replay that Session (child-owned events only), then route later events for that Session through the same streaming path the parent uses (`agent/assistant-stream` thinking and text, plus `session/event` tool cards and folds). Sibling and parent events do not append. Settlement of the viewed child does not pop the stack.
