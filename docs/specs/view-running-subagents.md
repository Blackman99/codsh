...
Keep the existing nested Child view as the product. As soon as an in-process child Session exists (`subagent/start`), promote the matching pending `subagent` / `subagent_fork` card into a view with the same dim `click to enter` line the finished continuable card already has. A click covers the parent with that child's transcript; the view streams thinking, text, and tool cards the way the parent does; Esc pops one stack level and uncovers the parent's history, scroll, and open folds. The view is read-only: typing still flashes that Esc returns. Approvals from in-process descendants use the same keyboard gate as the live agent, so a watched child that needs a grant can proceed. Fork views skip the inherited parent prefix. One-shot background jobs and Workflow/Ralph worker children stay non-views.
...
3. **Enter and live streaming.**
   Enter still `sessions.get`s the id; missing still flashes that the subagent is no longer running. On hit: push the stack, cover the current Viewport (history, scroll offset, and open folds stay on a stack), replay that Session into the empty buffer (child-owned events only), then route later events for that Session through the same streaming path the parent uses (`agent/assistant-stream` thinking and text, plus `session/event` tool cards and folds). Sibling events do not append. Parent events keep writing into the covered buffer. Settlement of the viewed child does not pop the stack. Esc uncovers the covered transcript rather than replaying the parent log.
