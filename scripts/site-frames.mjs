...
const COPY = {
  ...
  zh: {
    ...
    turns: {
      ...
      note: '往回滚过一次轮次边界，问出这几行的那条提问就钉在顶部；右侧一列时间线标出当前在哪一轮，底部那行说明退了多远——点它就回到最新。Shift+←/→ 在轮次之间跳。',
    },
    'tool-call': {
      tab: '工具卡片',
      title: '工具调用只占一行',
      note: '每次调用都经由它的 presenter 渲染成一行——标题、+n -m、状态——左侧还有一道标出块边界的竖线；diff 本身点一下或按 Ctrl+O 就展开。',
    },
    'fold-open': {
      tab: '思考',
      title: '思考在你读的时候是展开的',
      note: '思考直接流进对话，头上那行在结束时变成计时行，一直保持展开，直到下一条提问把它折起。现在点一下就折起，点计时行再展开；Ctrl+O 依然一次开合全部。',
    },
    todos: {
      tab: 'Todo',
      title: 'todo 常驻可见',
