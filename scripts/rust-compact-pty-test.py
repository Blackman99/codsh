...
""")
        ...
        try:
            ...
            assert 'advertised_context=128000 (config' in shown or 'advertised_context=unknown' in shown
            ...
            assert 'cli-mock' in shown
            assert 'compacted' in shown.lower() or 'compacting' in shown.lower()
        ...
        try:
            ...
            resumed.write(b'\r')
            shown = resumed.wait_visible('RUST_ACP_ANSWER', 20)
            assert 'purpose=compaction' in shown or 'compaction summary' in shown
        ...
        try:
            ...
            shown = failing.visible()
            while time.monotonic() < deadline:
                failing.pump()
                shown = failing.visible()
                if 'Original dsh records were not discarded' in shown or 'summarizer failed' in shown or 'Compaction failed' in shown or 'error' in shown.lower():
                    break
            assert 'TOKEN_KEEP_ORIGINAL' in shown, shown
            assert 'TOKEN_KEEP_SECOND' in shown, shown
            assert 'success' not in shown.lower() or 'summarizer' in shown.lower()
