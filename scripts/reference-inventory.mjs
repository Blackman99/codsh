...
function environmentControls(text) {
  ...
  for (const match of text.matchAll(/(?:\b(?:std::)?env::(?:var|var_os)|\b(?:var|var_os))\s*\(\s*"([^"\\=\0\r\n]+)"/gu)) add(match[1], match.index + match[0].indexOf('"'))
  for (const match of text.matchAll(/\bconst\s+(?:ENV_[A-Z0-9_]+|[A-Z0-9_]+_ENV(?:_VAR)?)\s*:\s*&str\s*=\s*"([^"\\=\0\r\n]+)"/gu)) add(match[1], match.index + match[0].indexOf('"'))
  for (const match of text.matchAll(/\.env\(\s*"([^"\\=\0\r\n]+)"\s*,/gu)) add(match[1], match.index + match[0].indexOf('"'))
  for (const match of text.matchAll(/\benv\.get\(\s*"([^"\\=\0\r\n]+)"|\benv_nonempty\(\s*\w+\s*,\s*"([^"\\=\0\r\n]+)"/gu)) add(match[1] ?? match[2], match.index + match[0].indexOf('"'))
  for (const loop of text.matchAll(/\bfor\s+(\w+)\s+in\s+\[([^\]]+)\]\s*\{([^}]+)\}/gu)) {
    ...
    for (const match of loop[2].matchAll(/"([^"\\=\0\r\n]+)"/gu)) add(match[1], offset + match.index)
  }
