...
describe.skipIf(process.platform === 'win32')('the first five minutes: timeline, folds, hover', () => {
  ...
  it('names the block the pointer rests on, and gives the row back', async () => {
    ...
    const output = await drivePty('reasoning', [
      ...
      // Resting on the folded thought: the chrome says what it is, how much it
    ...
    expect(named.some(row => /thinking · 2 lines · click to expand/u.test(row))).toBe(true)
  ...
  it('opens the folded thought a click lands on, and folds it again from inside', async () => {
    ...
    const run = await drivePtySteps('reasoning', [
      ['Welcome to codsh', `think it over${ENTER}`, 300],
      // The thought lands folded; a click on the clock opens it.
      ['CODE_CLI_ANSWER after thinking', clickOn('thought for'), 600],
      // A click anywhere inside it folds it back to that one row.
    ...
    const captured = (offset: number | undefined): string => Buffer.from(run.output).subarray(0, offset).toString()
    const opened = screenOf(captured(run.offsets[2]), -1).alternate
    const shut = screenOf(captured(run.offsets[3]), -1).alternate
  ...
  it('keeps a block the person opened by hand open across the next real turn', async () => {
    // A fresh thought lands folded; Ctrl+O opens it and that form is the
    ...
    // The first thought stays open by hand; the live turn's thought is folded.
  ...
  it('shows thinking folded by default and opens it on Ctrl+O', async () => {
    const run = await drivePtySteps('reasoning', [
      ['Welcome to codsh', `think it over${ENTER}`, 300],
    ...
    // offsets[k] is the moment step k+1 wrote: the screen after step k settled.
    const captured = (offset: number | undefined): string => Buffer.from(run.output).subarray(0, offset).toString()
    const folded = screenOf(captured(run.offsets[1]), -1).alternate
    const open = screenOf(captured(run.offsets[2]), -1).alternate
    // Settled: the clock, the deliberation behind it.
    ...
    // The only fold on screen is closed, so Ctrl+O opens it.
